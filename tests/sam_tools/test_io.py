...
import equilibria.sam_tools.io as io_module
...
def _to_cell_map(state: SAMTransformState) -> dict[tuple[tuple[str, str], tuple[str, str]], float]:
...
def test_io_write_gdx_returns_metadata(tmp_path: Path) -> None:
    ...
    state = SAMTransformState(
        matrix=np.array([[1.5, 0.0], [0.0, 2.5]], dtype=float),
        row_keys=[("AG", "gvt"), ("I", "agr")],
        col_keys=[("I", "agr"), ("AG", "row")],
        source_path=tmp_path / "in.gdx",
        source_format="gdx",
    )
...
def test_io_load_ieem_raw_excel_uses_sam_class(
    ...
    monkeypatch.setattr(io_module.SAM, "from_ieem_excel", staticmethod(_fake_from_ieem_excel))
