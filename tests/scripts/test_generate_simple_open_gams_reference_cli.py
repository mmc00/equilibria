...
def _load_module() -> Any:
    spec = importlib.util.spec_from_file_location(
        "generate_simple_open_gams_reference", SCRIPT
    )
...
def test_cli_main_skip_gams_builds_manifest(tmp_path: Path, monkeypatch: Any) -> None:
    ...
    assert manifest["closure_references"]["simple_open_default"]["results_gdx"][
        "path"
    ] == str(default_gdx.resolve())
    assert manifest["closure_references"]["flexible_external_balance"]["results_gdx"][
        "path"
    ] == str(flexible_gdx.resolve())
