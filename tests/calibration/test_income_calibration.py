...
def compare_with_gams(our_result):
            ...
            if sym.get("name") == param_name:
                ...
                for record in records:
                    record_indices = tuple(
                        str(i).lower() for i in record.get("indices", [])
                    )
    ...
    for h in households:
        ...
        if gams_yhko is not None:
            ...
            status = "✓" if diff < 0.01 else "✗"
            print(
                f"    {status} YHKO:  Our={our_yhko:>12,.2f}  GAMS={gams_yhko:>12,.2f}  Diff={diff:>10.2f} ({pct_diff:>5.2f}%)"
            )
            total_errors += diff
        ...
        if gams_yhlo is not None:
            ...
            status = "✓" if diff < 0.01 else "✗"
            print(
                f"    {status} YHLO:  Our={our_yhlo:>12,.2f}  GAMS={gams_yhlo:>12,.2f}  Diff={diff:>10.2f} ({pct_diff:>5.2f}%)"
            )
            total_errors += diff
            max_error = max(max_error, diff)
        ...
        if gams_yho is not None:
            ...
            status = "✓" if diff < 0.01 else "✗"
            print(
                f"    {status} YHO:   Our={our_yho:>12,.2f}  GAMS={gams_yho:>12,.2f}  Diff={diff:>10.2f} ({pct_diff:>5.2f}%)"
            )
            total_errors += diff
            max_error = max(max_error, diff)
    ...
    for var_name, our_value in gov_vars:
        ...
        if gams_value is not None:
            ...
            status = "✓" if diff < 0.01 else "✗"
            print(
                f"  {status} {var_name:8s}: Our={our_value:>12,.2f}  GAMS={gams_value:>12,.2f}  Diff={diff:>10.2f} ({pct_diff:>5.2f}%)"
            )
            total_errors += diff
            max_error = max(max_error, diff)
    ...
    if gams_ito is not None:
        ...
        status = "✓" if diff < 0.01 else "✗"
        print(
            f"  {status} ITO:     Our={our_result.ITO:>12,.2f}  GAMS={gams_ito:>12,.2f}  Diff={diff:>10.2f} ({pct_diff:>5.2f}%)"
        )
        total_errors += diff
        max_error = max(max_error, diff)
    ...
    print(
        f"  Status: {'✓ PASS' if max_error < 0.01 else '✗ FAIL - Differences detected'}"
    )
