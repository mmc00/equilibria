...
def test_final_calibration():
    ...
    print("=" * 70)
    final_calibrator = FinalCalibrator(
        sam_data, income_result, production_result, trade_result
    )
    ...
    gdp_pct = (
        (gdp_diff / final_result.GDP_BPO * 100) if final_result.GDP_BPO != 0 else 0
    )
    ...
    for h in final_calibrator.sets["H"]:
        gamma_sum = sum(
            final_result.gamma_LES.get((i, h), 0) for i in final_calibrator.sets["I"]
        )
        print(f"  {h}: gamma sum = {gamma_sum:.4f}")

    print("\n3. Final Demand Composition:")
    total_cons = sum(
        sum(final_result.CO.get((i, h), 0) for h in final_calibrator.sets["H"])
        for i in final_calibrator.sets["I"]
    )
    total_gov = sum(final_result.CGO.values())
