...
def test_report_records_first_dirty_layer():
    ...
    results = {
        "shock": [_lr("mcp_pairing", "clean", "continue"),
                  _lr("nl_compare", "dirty", "explain_stop")],
    }
    rep = build_report("gtap7_3x3", ["check", "shock"], ref=ref,
                       period_results=results, kkt_reader="pure-python")
...
def test_tree_shows_provenance_line():
    ...
    rep = build_report("gtap7_3x3", ["shock"], ref=ref,
                       period_results=results, kkt_reader="gdxdump-text")
...
def test_vacuous_layer_is_visible_in_report():
    ref = GdxResolution(Path("/ref/out.gdx"), "durable", "n", True)
    results = {"shock": [
        _lr("nl_compare", "error", "vacuous_continue",
            kind="no_common_constraints", headline="no common constraints")]}
    rep = build_report("gtap7_3x3", ["shock"], ref=ref,
                       period_results=results, kkt_reader="pure-python")
