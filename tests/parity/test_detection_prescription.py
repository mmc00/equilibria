...
def test_ran_but_not_converged_stays_hypothesis():
    ...
    v = make_validation(
        tool="probe",
        converged=False,
        predicted_effect="match up",
        predicted_effect_observed=False,
        evidence="code=2",
    )
    assert v is None
...
def test_converged_but_effect_absent_stays_hypothesis():
    ...
    v = make_validation(
        tool="probe",
        converged=True,
        predicted_effect="match>73%",
        predicted_effect_observed=False,
        evidence="code=1 but match 60.7%",
    )
...
def test_detection_can_be_firm_independent_of_prescription():
    ...
    with_diagnosis(
        ...
        detection=make_detection(
            "GAMS freezes pf", "var.fx(tsim-1)", confidence="firm"
        ),
