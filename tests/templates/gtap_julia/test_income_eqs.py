"""Task 10: income/welfare equations are satisfied at Julia's solution."""
...
@pytest.mark.slow
def test_income_residuals_zero_at_julia_solution(tmp_path):
    sol = load_solution(dump_solution(dataset="sample", out_dir=tmp_path))
    m = pyo.ConcreteModel()
    seed_model(m, sol)
    cons = build_group(m, sol, "income")
    assert len(cons) > 0
    ...
    assert worst < 1e-6, f"worst income residual {worst:.2e} at {worst_name}"
