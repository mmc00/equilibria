...
def _read_pyomo_origin(gdx, names):
    ...
    for line in txt.splitlines():
        sm = re.match(
            r"\s*(?:free|positive)?\s*Variable\s+([A-Za-z0-9_]+)\s*/L\s+([-\d.E+]+)",
            line,
        )
        if sm:
...
@pytest.mark.skipif(
    not Path(GAMS).exists() or not Path(GDXDUMP).exists(),
...
def test_pep_phase2_3_translation_faithful(tmp_path):
    ...
    state = PEPModelCalibrator(
        sam_file=root / "data/SAM-V2_0.gdx", val_par_file=root / "data/VAL_PAR.xlsx"
    ).calibrate()
    m = build_pep_model(state, variant="base", form="nlp")
    names = sorted(
        {v.name for v in m.component_objects(Var, active=True)}, key=len, reverse=True
    )
    ...
    r = subprocess.run(
        [GAMS, str(gms), f"gdx={origin}", "lo=2", "reslim=120"],
        capture_output=True,
        text=True,
        cwd=str(tmp_path),
    )
    ...
    assert len(keys) > 200, (
        f"too few comparable cells ({len(keys)}) — de-scalarize broke"
    )
    ...
    assert not bad, (
        f"phase-3 translation mismatches ({len(keys) - len(bad)}/{len(keys)}): {bad[:5]}"
    )
