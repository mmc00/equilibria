* ------------------------------------------------------------------------------
* GTAP calibration dump (Python parity helper)
*
* Usage:
*   gams scripts/cal_dump.gms
*
* Optional globals:
*   --baseName=9x10
*   --refRoot=src/equilibria/templates/reference/gtap
*   --inDir=src/equilibria/templates/reference/gtap/data
*   --outDir=src/equilibria/templates/reference/gtap/data
* ------------------------------------------------------------------------------

$if not setGlobal baseName  $setGlobal baseName  9x10
$if not setGlobal refRoot   $setGlobal refRoot   src/equilibria/templates/reference/gtap
$if not setGlobal scriptDir $setGlobal scriptDir %refRoot%/scripts
$if not setGlobal inDir     $setGlobal inDir     %refRoot%/data
$if not setGlobal outDir    $setGlobal outDir    %refRoot%/data
...
* Data and model scaffolding
$include "%scriptDir%/getData.gms"
...
$include "%scriptDir%/model.gms"
$include "%scriptDir%/cal.gms"

* ------------------------------------------------------------------------------
* Diagnostics for parity/debug:
*   - diag_counts(*)  : nonzero/base record counts by key symbol
*   - diag_scalars(*) : scalar diagnostics at calibration point
* ------------------------------------------------------------------------------
parameter
   diag_counts(*)
   diag_scalars(*)
...
* Optional fixed/free report includes (if available in scriptDir).
$ifThenI.reportExists exist "%scriptDir%/report_fixed_setup.inc"
$include "%scriptDir%/report_fixed_setup.inc"
$ifThenI.reportRunExists exist "%scriptDir%/report_fixed_run.inc"
$$batinclude "%scriptDir%/report_fixed_run.inc" check
$endIf.reportRunExists
$endIf.reportExists

* Dump all symbols (parameters + variable levels) after calibration.
* Diagnostics above are part of the symbol table and will be included.
