"""GTAP v6.2 monolith model equations — test-only form-diff oracle.

Ported from the orphan branch ``gtap/v62-multiperiod``
(``src/equilibria/templates/gtap_v62/gtap_v62_model_equations.py``) as a
known-good, already-validated-to-GEMPACK reference for the F7 GTAP6
blocks migration. This module is used ONLY by
``tests/blocks/gtap6/test_gtap6_blocks_form.py`` to form-diff the new
Block-subclass implementations (Tasks 6-9) against this monolith. It is
never imported by ``templates/gtap6/`` or ``blocks/gtap6/`` and will be
deleted once all datasets pass the final gate.
...
from equilibria.templates.gtap6.gtap6_calibration import (
    DerivedGTAP6Calibration,
    derive_calibration,
)
...
GTAP6_TAX_STREAMS = ("imptx", "exptx", "outtx", "indtx", "facttx", "subdy")


class GTAP6MonolithOracle:
    """Build a Pyomo model for GTAP v6.2.

    Args:
        sets: Loaded :class:`GTAP6Sets`.
        params: Loaded :class:`GTAP6Parameters`.
        closure: Closure configuration (defaults to the standard v6.2
            closure with ``pgdpwld`` numeraire).
        derived: Pre-computed :class:`DerivedGTAP6Calibration`. If not
            supplied, :func:`derive_calibration` is invoked at
            construction time.

    Example::

        sets = GTAP6Sets(); sets.load_from_har(...)
        params = GTAP6Parameters(); params.load_from_har(..., sets=sets)
        eqs = GTAP6MonolithOracle(sets, params)
    ...
    def __init__(
        ...
        derived: DerivedGTAP6Calibration | None = None,
        mode: str = "nlp",
    ) -> None:
        """
        Args:
        ...
        self.sets = sets
        self.params = params
        self.closure = closure or GTAP6ClosureConfig()
        self.derived = (
            derived if derived is not None else derive_calibration(sets, params)
        )
        self.mode = mode
    ...
    def build_model(self) -> ConcreteModel:
        ...
        Builds sets, parameters, and variables, then populates all
        equations via :meth:`_add_equations`.
        """
        from pyomo.environ import ConcreteModel
    ...
    def _add_sets(self, model: ConcreteModel) -> None:
        ...
        model.j = Set(
            initialize=self.sets.prod_comm, doc="Producing sectors (PROD_COMM)"
        )
        # Tax streams (single-aggregate per region in v6.2)
        model.gy = Set(initialize=GTAP6_TAX_STREAMS, doc="Tax revenue streams")
    ...
    def _add_parameters(self, model: ConcreteModel) -> None:
        ...
        model.alpha_dom_hhd = Param(
            model.i,
            model.r,
            initialize=dict(c.alpha_dom_hhd),
            default=0.0,
            mutable=False,
        ...
        model.alpha_imp_hhd = Param(
            model.i,
            model.r,
            initialize=dict(c.alpha_imp_hhd),
            default=0.0,
            mutable=False,
        ...
        model.share_hhd_cd = Param(
            model.i,
            model.r,
            initialize=dict(c.share_hhd_cd),
            default=0.0,
            mutable=False,
        ...
        model.alpha_dom_gov = Param(
            model.i,
            model.r,
            initialize=dict(c.alpha_dom_gov),
            default=0.0,
            mutable=False,
        ...
        model.alpha_imp_gov = Param(
            model.i,
            model.r,
            initialize=dict(c.alpha_imp_gov),
            default=0.0,
            mutable=False,
        ...
        model.share_gov_cd = Param(
            model.i,
            model.r,
            initialize=dict(c.share_gov_cd),
            default=0.0,
            mutable=False,
    ...
    def _add_variables(self, model: ConcreteModel) -> None:
        ...
        model.qo = Var(
            ...
            bounds=(lb, None),
            initialize=lambda m, j, r: _init_q(
                c.vop.get((j, r), c.vom.get((j, r), 1.0))
            ),
        ...
        # (see :data:`DerivedGTAP6Calibration.pf_int_0`); this guarantees
        ...
        model.ppm = Var(
            ...
            within=NonNegativeReals,
            bounds=(lb, None),
            initialize=lambda m, i, r: c.pim_0.get((i, r), 1.0)
            * (1.0 + c.tpi.get((i, r), 0.0)),
            doc="ppm(i,r) — household imported agent price (= pim * (1+tpi))",
        )
        # Phase 3.20 CDE normalization: at benchmark up_0 = 1 and
        # pcons_0 = 1 (the dual deflator). The CDE expenditure-function
        # identity (eq_pcons) requires up_0 = 1 for sum_i CONSHR_i_0 = 1
        # to hold; the welfare identity (eq_up: pcons * up = yp/yp_0)
        # then pins pcons_0 = 1.
        model.pcons = Var(
            model.r,
            within=NonNegativeReals,
            bounds=(lb, None),
            initialize=lambda m, r: 1.0,
            doc="pcons(r) — CDE expenditure deflator (yp / (yp_0 * up))",
        )
        model.up = Var(
            model.r,
            within=NonNegativeReals,
            bounds=(lb, None),
            initialize=lambda m, r: 1.0,
            doc="up(r) — private utility (Phase 3.20 levels-CDE normalisation)",
        )
        ...
        model.qxs = Var(
            ...
            within=NonNegativeReals,
            bounds=(lb, None),
            initialize=lambda m, i, s, rp: max(
                c.qxs_0.get((i, s, rp), b.vxwd.get((i, s, rp), 0.0)), lb
            ),
        ...
        model.qst = Var(
            ...
            within=NonNegativeReals,
            bounds=(lb, None),
            initialize=lambda m, mg, r: max(
                c.qst_0.get((mg, r), b.vst.get((mg, r), 0.0)), lb
            ),
    ...
    def _add_equations(self, model: ConcreteModel) -> None:
        ...
        Populates the **production block**, the **household + government
        demand block** and **CGDS identities**, the **trade block**
        (pricing chain + bottom Armington), the **margins block**
        (Cobb-Douglas), **commodity market clearing**, the **factor
        markets** (mobile wage clearing + sluggish CET), the **income
        block** (regional income, household and gov income identities,
        tax revenue per stream), and the **closure** (numeraire and
        Walras).

        Known benchmark residuals (BOOK3X3):
    ...
    def _add_production_block(self, model: ConcreteModel) -> None:
        ...
        def eq_qo_rule(m, j, r):
            if (
                pyo_value(m.vop[j, r]) <= 1e-8
                if hasattr(m, "vop")
                else pyo_value(m.vom[j, r]) <= 1e-8
            ):
        ...
        def eq_va_rule(m, j, r):
            ...
            sigma_top = _ces_cd_sigma(sigma_top)
            return (
                m.va[j, r] == sva * m.qo[j, r] * (m.ps[j, r] / m.pva[j, r]) ** sigma_top
            )
        ...
        def eq_qf_rule(m, i, j, r):
            ...
            return (
                m.qf[i, j, r]
                == sint * m.qo[j, r] * (m.ps[j, r] / m.pf_int[i, j, r]) ** sigma_top
            )
        ...
        def eq_qfe_rule(m, f, j, r):
            ...
            return (
                m.qfe[f, j, r]
                == sfac * m.va[j, r] * (m.pva[j, r] / m.pfe[f, j, r]) ** sigma_va
            )
        ...
        def eq_pf_int_rule(m, i, j, r):
            ...
            return (
                m.pf_int[i, j, r] ** exp
                == ad * m.pfd[i, j, r] ** exp + ai * m.pfm[i, j, r] ** exp
            )
        ...
        def eq_qfd_rule(m, i, j, r):
            ...
            return (
                m.qfd[i, j, r]
                == ad * m.qf[i, j, r] * (m.pf_int[i, j, r] / m.pfd[i, j, r]) ** sigma_d
            )
        ...
        def eq_qfm_rule(m, i, j, r):
            ...
            return (
                m.qfm[i, j, r]
                == ai * m.qf[i, j, r] * (m.pf_int[i, j, r] / m.pfm[i, j, r]) ** sigma_d
            )
    ...
    def _add_household_demand_block(self, model: ConcreteModel) -> None:
        ...
        def eq_pp_rule(m, i, r):
            ...
            exp = 1.0 - sigma_d
            return (
                m.pp[i, r] ** exp == ad * m.ppd[i, r] ** exp + ai * m.ppm[i, r] ** exp
            )

        model.eq_pp = Constraint(model.i, model.r, rule=eq_pp_rule)

        # eq_qp: CDE levels demand (Phase 3.20). The expenditure share
        # COMSHR_i(up, pp, yp) is endogenous — unlike Phase 3.19 the
        # CDE coefficients update naturally as the solver moves state.
        #
        #   share_i ≡ pp_i * qp_i / yp
        #           = CONSHR_i_0
        #             * up^(INCPAR_i * SUBPAR_i)
        #             * ((pp_i / pp_i_0) / (yp / yp_0))^SUBPAR_i
        #
        # At benchmark up=1, pp=pp_0, yp=yp_0 ⇒ share = CONSHR_0 ⇒ qp = qp_0.
        # The expenditure-function identity (eq_cde_expfn below) gives
        # sum_i share_i = 1 and implicitly defines `up`.
        derived = self.derived
        params_e = self.params.elasticities
        eps_q = 1e-12
        eps_p = 1e-12
        ...
        def _cde_term(m, i, r):
            """One summand of the CDE expenditure function /
            expenditure-share expression:
                CONSHR_i_0 * up^(INCPAR*SUBPAR)
                          * ((pp_i/pp_i_0) / (yp/yp_0))^SUBPAR_i
            Returns (share_expression, conshr_0)."""
            cshr_0 = float(derived.share_hhd_cd.get((i, r), 0.0))
            if cshr_0 <= 0.0:
                return None, 0.0
            subp = float(params_e.subpar.get((i, r), 0.0))
            incp = float(params_e.incpar.get((i, r), 1.0))
            pp_i_0 = _safe_pos(float(derived.pp_0.get((i, r), 1.0)))
            yp_0 = _safe_pos(float(self.derived.yp_0.get(r, 1.0)))
            # Numerically-friendly form: ratios are O(1) at benchmark
            # and remain O(1) under realistic shocks.
        ...
        def eq_qp_rule(m, i, r):
            ...
            if share_expr is None:
                # CONSHR_0 = 0 ⇒ no consumption ⇒ pin to zero.
                return m.qp[i, r] == 0.0
            # share_i = pp_i * qp_i / yp ⇒ pp_i * qp_i = yp * share_i.
        ...
        def eq_qpd_rule(m, i, r):
            ...
            sigma_d = _ces_cd_sigma(float(pyo_value(m.esubd[i])))
            return (
                m.qpd[i, r] == ad * m.qp[i, r] * (m.pp[i, r] / m.ppd[i, r]) ** sigma_d
            )
        ...
        def eq_qpm_rule(m, i, r):
            ...
            sigma_d = _ces_cd_sigma(float(pyo_value(m.esubd[i])))
            return (
                m.qpm[i, r] == ai * m.qp[i, r] * (m.pp[i, r] / m.ppm[i, r]) ** sigma_d
            )
        ...
        # eq_pcons: CDE expenditure-function identity (Phase 3.20).
        # The expenditure function (HT F1-F3, Hanoch 1975) implicitly
        # defines up via
        #
        #   sum_i CONSHR_i_0
        #         * up^(INCPAR_i*SUBPAR_i)
        #         * ((pp_i/pp_i_0)/(yp/yp_0))^SUBPAR_i  =  1.
        #
        # At benchmark up=1, ratio=1 ⇒ sum_i CONSHR_i_0 = 1 ✓.
        # Under shock the share allocation among goods adjusts as up,
        # pp, and yp move — this is what makes the CDE coefficients
        # "state-dependent" in the way GEMPACK's Gragg-multi does
        # through coefficient refresh per substep.
        #
        # We re-use the variable name `pcons` to host this identity for
        # closure-matching reasons; the conventional consumption price
        # index is now derived as pcons_deflator = yp / (yp_0 * up).
    ...
    def _add_government_demand_block(self, model: ConcreteModel) -> None:
        ...
        def eq_pg_rule(m, i, r):
            ...
            exp = 1.0 - sigma_d
            return (
                m.pg[i, r] ** exp == ad * m.pgd[i, r] ** exp + ai * m.pgm[i, r] ** exp
            )
        ...
        def eq_qgd_rule(m, i, r):
            ...
            sigma_d = _ces_cd_sigma(float(pyo_value(m.esubd[i])))
            return (
                m.qgd[i, r] == ad * m.qg[i, r] * (m.pg[i, r] / m.pgd[i, r]) ** sigma_d
            )
        ...
        def eq_qgm_rule(m, i, r):
            ...
            sigma_d = _ces_cd_sigma(float(pyo_value(m.esubd[i])))
            return (
                m.qgm[i, r] == ai * m.qg[i, r] * (m.pg[i, r] / m.pgm[i, r]) ** sigma_d
            )
    ...
    def _add_trade_block(self, model: ConcreteModel) -> None:
        ...
        def eq_pe_rule(m, i, s, d):
            if (
                pyo_value(m.qxs[i, s, d]) <= 1e-8
                or float(pyo_value(m.alpha_xs[i, s, d])) <= 0.0
            ):
                return Constraint.Skip
        ...
        def eq_pim_rule(m, i, d):
            terms = [
                ...
                for s in m.s
                if pyo_value(m.alpha_xs[i, s, d]) > 0.0
            ]
            if not terms:
                return Constraint.Skip
        ...
        def eq_qxs_rule(m, i, s, d):
            ...
            return (
                m.qxs[i, s, d]
                == ax * m.qim[i, d] * (m.pim[i, d] / m.pms[i, s, d]) ** sigma_m
            )
    ...
    def _add_income_and_closure(self, model: ConcreteModel) -> None:
        ...
        def eq_y_rule(m, r):
            if pyo_value(m.y_0[r]) <= 1e-8:
                return Constraint.Skip
            factor_inc = sum(
                m.pf[f, r] * m.qoes[f, r] for f in m.f if pyo_value(m.evom[f, r]) > 0.0
            )
        ...
        def eq_tax_revenue_rule(m, r):
            ...
            tfu = sum(
                m.tf[f, j, r] * m.pf[f, r] * m.qfe[f, j, r] for f in m.f for j in m.j
            )
            tout = sum(m.to[i, r] * m.ps[i, r] * m.qo[i, r] for i in m.j)
            tex = sum(
                m.txs[i, r, d] * m.ps[i, r] * m.qxs[i, r, d] for i in m.i for d in m.rp
            )
            tim = sum(
...
def build_monolith_model(dataset_dir, *, mode: str = "nlp"):
    ...
    sets = GTAP6Sets()
    sets.load_from_har(
        dataset_dir / "sets.har", default_path=dataset_dir / "default.prm"
    )
    params = GTAP6Parameters()
    params.load_from_har(
        dataset_dir / "basedata.har", dataset_dir / "default.prm", sets
    )
