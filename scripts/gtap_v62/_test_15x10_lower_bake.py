"""Test bake_tolerance sweep on gtap6_15x10 with EQUAL tolerance both directions.

Hypothesis: 15x10 SAM has imperfections at ~5e-4 level on eq_qim/eq_pfe.
Bake tolerance 1e-3 LEAVES them active (only |resid| > tol gets baked).
Lower the bake tolerance so they get absorbed.
"""
import os, sys, time
...
for tol in [1e-5, 1e-6]:
    print(f"\n=== gtap6_15x10 bake_tolerance={tol:g} ===", flush=True)
    sets = GTAPv62Sets()
    ...
    pipe = apply_v62_pipeline(model, mode="mcp", bake_tolerance=tol, params=params,
                              drop_dead_rows_threshold=1.0e-6)
    print(f"  free={pipe['free_vars']} baked={pipe['prebalance']['n_baked']} "
          f"max_baked={pipe['prebalance']['max_abs_baked']:.2e}", flush=True)
    ...
    print(f"  PATH BASELINE: tc={res.termination_code} r={res.residual:.4e} "
