...
| **gtap7_3x3 (pure-gtap real-CES SOLVE, ifSUB=0)** | — | **CHECK 100% / SHOCK 99.70% (tol 1%), code=1/1/1** |
| **gtap7_3x3 (pure-gtap real-CES SOLVE, ifSUB=1)** | — | **CHECK 100% / SHOCK 98.95% (tol 1%), code=1/1/1** |
| **gtap7_15x10 (altertax, ifSUB=0/1)** | — | **99.5% / 99.5% (tol 1%), code=1/1/1, floors 99/99** |

**gtap-puro ifSUB=1 shock cerrado (2026-07-01): 55.63% → 98.95%, fiel a GAMS, sin hardcodeo, cero regresión (commit `982e47f`).** DOS eslabones, ambos bugs de PAREO estructural (no ecuaciones): (1) el precio de import bajo ifSUB=1 es el macro `_m_pm` horneado con imptx BASE en eq_xweq/eq_pmteq → `_rebuild_import_demand_shock_ifsub` lo reconstruye sobre las Vars VIVAS `pe`+`ptmg` (55→76%, =CONVERT e544/e553); (2) el matcher Hopcroft-Karp emparejaba los precios a las CANTIDADES, dejando `eq_xseq` (`xs==xds+xet`, una free-row de GAMS) sin par → se desactivaba → el balance de oferta se rompía (xds+xet>xs) → toda la cadena de precios de EU_28 caía −13% a una raíz espuria (76→98.95%). Fix: mantener eq_xseq activa (`protect_xseq`) + forzar el ciclo de pareo de GAMS (pseq↔ps/xdseq↔xds/pdeq↔pd/xeteq↔xet/peteq↔pet/xseq↔xs) con hard-pairs que saltan la adyacencia (la complementariedad cantidad⊥precio del MCP que un sistema cuadrado no expresa). Gated a gtap-mode+ifSUB=1. **Diagnóstico decisivo = CONVERT+seed**: el sistema de GAMS convertido a Pyomo, evaluado en el punto de Python, viola 71 ecuaciones (residual 2.6) → NO era multi-equilibrio sino pareo distinto. Ese test discrimina "multi-eq vs pareo" de primera mano (correr ANTES de agotar holdfixes por variable — fijar pfact/pd/pft a GAMS no hacía nada, la región era auto-consistente en el nivel equivocado).
