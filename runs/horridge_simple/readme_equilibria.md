...
## To complete on Windows (needs GEMPACK + GAMS)

```bat
gempack.bat                    :: tablo + gemsim -> fixcap.sl4 (+ .har)
:: convert fixcap.sl4 -> sl4dump.har via sltoht (see equilibria guide §8 chain)
dogams.bat                     :: -> ResultsMCP.gdx, ResultsNLP.gdx
python compare_gams_vs_gempack.py --gempack sl4dump.har --mcp ResultsMCP.gdx --nlp ResultsNLP.gdx
```

Expected: the GAMS-vs-GEMPACK block reports ~99–100% within 1pp on this small shock —
the reference number that contextualizes the matrix page's ~52% at a large shock.
