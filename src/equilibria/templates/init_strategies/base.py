...
class InitStrategySolverProtocol(Protocol):
    ...
    def _apply_equation_consistent_adjustments(
        self, vars: PEPModelVariables
    ) -> None: ...
    ...
    def _apply_trade_blockwise_transformation(
        self, vars: PEPModelVariables
    ) -> None: ...

    def _apply_production_blockwise_accounting(
        self, vars: PEPModelVariables
    ) -> None: ...

    def _apply_commodity_balance_blockwise(self, vars: PEPModelVariables) -> None: ...

    def _apply_trade_market_clearing_blockwise(
        self, vars: PEPModelVariables
    ) -> None: ...

    def _attempt_coupled_trade_reconciliation(
        self, vars: PEPModelVariables
    ) -> None: ...
