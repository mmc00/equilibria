...
def attach_all_blocks(m, S, P, idx, variant) -> int:
    ...
    def eq3(m, j):
        ...
        return m.VA[j] == B * (
            bv * m.LDC[j] ** (-rho) + (1 - bv) * m.KDC[j] ** (-rho)
        ) ** (-1.0 / rho)
    ...
    def eq5(m, j):
        ...
        if B == 0:
            return Constraint.Skip
        return m.LDC[j] == B * sum(
            p("beta_LD", l, j) * m.LD[l, j] ** (-rho) for l in L if (l, j) in LDact
        ) ** (-1.0 / rho)
    ...
    def eq6(m, l, j):
        ...
        return (
            m.LD[l, j]
            == (p("beta_LD", l, j) * m.WC[j] / m.WTI[l, j]) ** sg
            * p("B_LD", j) ** (sg - 1)
            * m.LDC[j]
        )
    ...
    def eq7(m, j):
        ...
        if B == 0 or not any(
            kk == j for (_, kk) in [(k, j) for k in K if (k, j) in KDact]
        ):
            return Constraint.Skip
        return m.KDC[j] == B * sum(
            p("beta_KD", k, j) * m.KD[k, j] ** (-rho) for k in K if (k, j) in KDact
        ) ** (-1.0 / rho)
    ...
    def eq8(m, k, j):
        ...
        return (
            m.KD[k, j]
            == (p("beta_KD", k, j) * m.RC[j] / m.RTI[k, j]) ** sg
            * p("B_KD", j) ** (sg - 1)
            * m.KDC[j]
        )
    ...
    add(
        ...
        == sum(
            p("lambda_WL", h, l)
            * m.W[l]
            * sum(m.LD[l, j] for j in J if (l, j) in LDact)
            for l in L
        ),
    )
    add(
        ...
        == sum(
            p("lambda_RK", h, k)
            * sum(m.R[k, j] * m.KD[k, j] for j in J if (k, j) in KDact)
            for k in K
        ),
    )
    ...
    add(
        "eq15",
        H,
        lambda m, h: m.CTH[h] == m.YDH[h] - m.SH[h] - sum(m.TR[ag, h] for ag in AGNG),
    )
    add(
        "eq16",
        H,
        lambda m, h: m.SH[h]
        == m.PIXCON ** p("eta") * p("sh0", h) + p("sh1", h) * m.YDH[h],
    )
    ...
    add(
        "eq22",
        None,
        lambda m: m.YG == m.YGK + m.TDHT + m.TDFT + m.TPRODN + m.TPRCTS + m.YGTR,
    )
    add(
        "eq23",
        None,
    ...
    add("eq24", None, lambda m: sum(m.TDH[h] for h in H) == m.TDHT)
    add("eq25", None, lambda m: sum(m.TDF[f] for f in F) == m.TDFT)
    add("eq26", None, lambda m: m.TPRODN == m.TIWT + m.TIKT + m.TIPT)
    add("eq27", None, lambda m: sum(m.TIW[l, j] for (l, j) in LDact) == m.TIWT)
    add("eq28", None, lambda m: sum(m.TIK[k, j] for (k, j) in KDact) == m.TIKT)
    add("eq29", None, lambda m: sum(m.TIP[j] for j in J) == m.TIPT)
    add("eq30", None, lambda m: m.TPRCTS == m.TICT + m.TIMT + m.TIXT)
    add("eq31", None, lambda m: sum(m.TIC[i] for i in I) == m.TICT)
    add("eq32", None, lambda m: sum(m.TIM[i] for i in IMact) == m.TIMT)
    add("eq33", None, lambda m: sum(m.TIX[i] for i in EXDact) == m.TIXT)
    add("eq34", None, lambda m: sum(m.TR["gvt", ag] for ag in AGNG) == m.YGTR)
    add(
        "eq35",
        H,
        lambda m, h: m.TDH[h]
        == m.PIXCON ** p("eta") * p("ttdh0", h) + p("ttdh1", h) * m.YH[h],
    )
    add(
        "eq36",
        F,
        lambda m, f: m.TDF[f]
        == m.PIXCON ** p("eta") * p("ttdf0", f) + p("ttdf1", f) * m.YFK[f],
    )
    add(
        "eq37",
        LDact,
        lambda m, l, j: m.TIW[l, j] == p("ttiw", l, j) * m.W[l] * m.LD[l, j],
    )
    add(
        "eq38",
        KDact,
        lambda m, k, j: m.TIK[k, j] == p("ttik", k, j) * m.R[k, j] * m.KD[k, j],
    )
    ...
    add(
        "eq41",
        IMact,
        lambda m, i: m.TIM[i] == p("ttim", i) * m.e * p("PWM", i) * m.IM[i],
    )
    add(
        "eq42",
        EXDact,
        lambda m, i: m.TIX[i]
        == p("ttix", i)
        * (m.PE[i] + sum(m.PC[ij] * p("tmrg_X", ij, i) for ij in I))
        * m.EXD[i],
    )
    add(
        "eq43", None, lambda m: m.YG - sum(m.TR[ag, "gvt"] for ag in AGNG) - m.G == m.SG
    )
    ...
    add(
        "eq44",
        None,
        lambda m: sum(m.e * p("PWM", i) * m.IM[i] for i in IMact)
        + sum(
            p("lambda_RK", "row", k)
            * sum(m.R[k, j] * m.KD[k, j] for j in J if (k, j) in KDact)
            for k in K
    ...
    add(
        "eq47",
        [(ag, h) for ag in AGNG for h in H],
        lambda m, ag, h: m.TR[ag, h] == p("lambda_TR_households", ag, h) * m.YDH[h],
    )
    add(
        "eq48",
        H,
        lambda m, h: m.TR["gvt", h]
        == m.PIXCON ** p("eta") * p("tr0", h) + p("tr1", h) * m.YH[h],
    )
    add(
        "eq49",
        [(ag, f) for ag in AG for f in F],
        lambda m, ag, f: m.TR[ag, f] == p("lambda_TR_firms", ag, f) * m.YDF[f],
    )
    add(
        "eq50",
        AGNG,
        lambda m, ag: m.TR[ag, "gvt"] == m.PIXCON ** p("eta") * p("TRO", ag, "gvt"),
    )
    add(
        "eq51",
        AGD,
        lambda m, ag: m.TR[ag, "row"] == m.PIXCON ** p("eta") * p("TRO", ag, "row"),
    )

    # ---------------- DEMAND (EQ52-EQ57) ----------------
    add(
        "eq52",
        [(i, h) for i in I for h in H],
        lambda m, i, h: m.PC[i] * m.C[i, h]
        == m.PC[i] * m.CMIN[i, h]
        + p("gamma_LES", i, h) * (m.CTH[h] - sum(m.PC[ij] * m.CMIN[ij, h] for ij in I)),
    )
    add("eq53", None, lambda m: m.IT - sum(m.PC[i] * m.VSTK[i] for i in I) == m.GFCF)
    ...
    add(
        ...
        == sum(
            ...
            + p("tmrg_X", i, ij) * (m.EXD[ij] if ij in EXDact else 0.0)
            for ij in I
        ),
    )
    ...
    def eq58(m, j):
        ...
        if B == 0 or not prods:
            return Constraint.Skip
        return m.XST[j] == B * sum(
            p("beta_XT", j, i) * m.XS[j, i] ** rho for i in prods
        ) ** (1.0 / rho)
    ...
    add(
        ...
        EXDact,
        lambda m, i: m.EXD[i]
        == p("EXDO", i) * (m.e * p("PWX", i) / m.PE_FOB[i]) ** p("sigma_XD"),
    )
    ...
    add(
        "eq65",
        J,
        lambda m, j: m.PP[j] * m.XST[j] == m.PVA[j] * m.VA[j] + m.PCI[j] * m.CI[j],
    )
    add("eq66", J, lambda m, j: m.PT[j] == (1 + p("ttip", j)) * m.PP[j])
    add(
        "eq67",
        J,
        lambda m, j: m.PCI[j] * m.CI[j] == sum(m.PC[i] * m.DI[i, j] for i in I),
    )
    add(
        "eq68",
        J,
        lambda m, j: m.PVA[j] * m.VA[j] == m.WC[j] * m.LDC[j] + m.RC[j] * m.KDC[j],
    )
    # GAMS instantiates EQ70 over ALL (l,j) (WTI defined for every labor type × sector),
    # not just the LDO-active mask.
    add(
        "eq70",
        [(l, j) for l in L for j in J],
    ...
    add(
        "eq73",
        KDact,
        lambda m, k, j: (m.R[k, j] == m.RK[k]) if p("kmob") else Constraint.Skip,
    )
    ...
    add(
        ...
        DDact,
        lambda m, i: m.PD[i]
        == (m.PL[i] + sum(m.PC[ij] * p("tmrg", ij, i) for ij in I))
        * (1 + p("ttic", i)),
    )
    add(
        "eq78",
        ...
        lambda m, i: m.PM[i]
        == (
            (1 + p("ttim", i)) * m.e * p("PWM", i)
            + sum(m.PC[ij] * p("tmrg", ij, i) for ij in I)
        )
        * (1 + p("ttic", i)),
    )
    ...
    def eq80(m):
        ...
        num1 = sum(
            ((m.PVA[j] * m.VA[j] + m.TIP[j]) / m.VA[j]) * p("VAO", j) for j in jset
        )
        num2 = sum(((m.PVA[j] * m.VA[j] + m.TIP[j]) / m.VA[j]) * m.VA[j] for j in jset)
        den1 = sum(
            ((p("PVAO", j) * p("VAO", j) + p("TIPO", j)) / p("VAO", j)) * p("VAO", j)
            for j in jset
        )  # constant
        den2 = sum(
            ((p("PVAO", j) * p("VAO", j) + p("TIPO", j)) / p("VAO", j)) * m.VA[j]
            for j in jset
        )
        return ((num1 / den1) * (num2 / den2)) ** 0.5 == m.PIXGDP
    ...
    add(
        "eq81",
        None,
        lambda m: m.PIXCON * sum(p("PCO", i) * sum(p("CO", i, h) for h in H) for i in I)
    ...
    add(
        "eq82",
        None,
        lambda m: exp(sum(p("gamma_INV", i) * log(m.PC[i] / p("PCO", i)) for i in I))
        == m.PIXINV,
    )
    add(
        "eq83",
        None,
        lambda m: exp(sum(p("gamma_GVT", i) * log(m.PC[i] / p("PCO", i)) for i in I))
        == m.PIXGVT,
    )
    ...
    add(
        "eq87",
        None,
        lambda m: sum(m.SH[h] for h in H) + sum(m.SF[f] for f in F) + m.SG + m.SROW
        == m.IT,
    )
    add(
        "eq84",
    ...
    add(
        "eq88",
        DDact,
        lambda m, i: sum(m.DS[j, i] for j in J if (j, i) in DSact) == m.DD[i],
    )
    add(
        "eq89",
        EXDact,
        lambda m, i: sum(m.EX[j, i] for j in J if (j, i) in EXact) == m.EXD[i],
    )
    ...
    add("eq90", None, lambda m: sum(m.PVA[j] * m.VA[j] for j in J) + m.TIPT == m.GDP_BP)
    add("eq91", None, lambda m: m.GDP_MP == m.GDP_BP + m.TPRCTS)
    add(
        "eq92",
        None,
        lambda m: sum(m.W[l] * m.LD[l, j] for (l, j) in LDact)
        + sum(m.R[k, j] * m.KD[k, j] for (k, j) in KDact)
        + m.TPRODN
        + m.TPRCTS
        == m.GDP_IB,
    )
    add(
        ...
        None,
        lambda m: sum(
            m.PC[i] * (sum(m.C[i, h] for h in H) + m.CG[i] + m.INV[i] + m.VSTK[i])
            for i in I
        )
        + sum(m.PE_FOB[i] * m.EXD[i] for i in EXDact)
