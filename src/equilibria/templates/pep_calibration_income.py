...
class IncomeCalibrationResult(BaseModel):
    ...
    YHKO: dict[str, float] = Field(
        default_factory=dict, description="Capital income by household"
    )
    YHLO: dict[str, float] = Field(
        default_factory=dict, description="Labor income by household"
    )
    YHTRO: dict[str, float] = Field(
        default_factory=dict, description="Transfer income by household"
    )
    YHO: dict[str, float] = Field(
        default_factory=dict, description="Total income by household"
    )
    YDHO: dict[str, float] = Field(
        default_factory=dict, description="Disposable income by household"
    )
    CTHO: dict[str, float] = Field(
        default_factory=dict, description="Total consumption by household"
    )
    TDHO: dict[str, float] = Field(
        default_factory=dict, description="Direct taxes by household"
    )
    SHO: dict[str, float] = Field(
        default_factory=dict, description="Savings by household"
    )

    # Firm incomes (lines 436-439)
    YFKO: dict[str, float] = Field(
        default_factory=dict, description="Capital income by firm"
    )
    YFTRO: dict[str, float] = Field(
        default_factory=dict, description="Transfer income by firm"
    )
    YFO: dict[str, float] = Field(
        default_factory=dict, description="Total income by firm"
    )
    YDFO: dict[str, float] = Field(
        default_factory=dict, description="Disposable income by firm"
    )
    TDFO: dict[str, float] = Field(
        default_factory=dict, description="Direct taxes by firm"
    )
    ...
    TIWO: dict[tuple[str, str], float] = Field(
        default_factory=dict, description="Labor tax by category and sector"
    )
    TIKO: dict[tuple[str, str], float] = Field(
        default_factory=dict, description="Capital tax by category and sector"
    )
    ...
    lambda_RK: dict[tuple[str, str], float] = Field(
        default_factory=dict, description="Capital income shares"
    )
    lambda_WL: dict[tuple[str, str], float] = Field(
        default_factory=dict, description="Labor income shares"
    )
    lambda_TR_households: dict[tuple[str, str], float] = Field(
        default_factory=dict, description="Transfer shares to households"
    )
    lambda_TR_firms: dict[tuple[str, str], float] = Field(
        default_factory=dict, description="Transfer shares to firms"
    )
    sh1O: dict[str, float] = Field(
        default_factory=dict, description="Marginal savings propensity"
    )
    tr1O: dict[str, float] = Field(
        default_factory=dict, description="Marginal transfer rate"
    )
...
class IncomeCalibrator:
    ...
    def _calibrate_household_incomes(self) -> None:
        ...
        for h in H:
            ...
            tro_h_out = sum(
                self._get_sam_value("SAM", "AG", ag, "AG", h) for ag in AGNG
            )
    ...
    def _calibrate_firm_incomes(self) -> None:
        ...
        for f in F:
            ...
            yfko = sum(
                self._get_sam_value("SAM", "AG", f_upper, "K", k.upper()) for k in K
            )
            ...
            yftro = sum(
                self._get_sam_value("SAM", "AG", f_upper, "AG", ag.upper()) for ag in AG
            )
    ...
    def _calibrate_government_income(self) -> None:
        ...
        ygko = sum(self._get_sam_value("SAM", "AG", "GVT", "K", k.upper()) for k in K)
        self.result.YGKO = ygko

        # TDHTO = total household direct taxes
        # TDHO(h) = SAM('AG', 'TD', 'AG', h)
        tdhto = sum(self._get_sam_value("SAM", "AG", "TD", "AG", h.upper()) for h in H)
        self.result.TDHTO = tdhto

        # TDFTO = total firm direct taxes
        # TDFO(f) = SAM('AG', 'TD', 'AG', f)
        tdfto = sum(self._get_sam_value("SAM", "AG", "TD", "AG", f.upper()) for f in F)
        self.result.TDFTO = tdfto

        # TICTO = total indirect commodity taxes
        # TICO(i) = SAM('Agents','ti','I',i) = SAM('AG', 'TI', 'I', i)
        ticto = sum(self._get_sam_value("SAM", "AG", "TI", "I", i.upper()) for i in I)
        self.result.TICTO = ticto

        # TIMTO = total import taxes
        # TIMO(i) = SAM('Agents','tm','I',i) = SAM('AG', 'TM', 'I', i)
        timto = sum(self._get_sam_value("SAM", "AG", "TM", "I", i.upper()) for i in I)
        self.result.TIMTO = timto

        # TIXTO = total export taxes
        # TIXO(i) = SAM('AG', 'GVT', 'X', i)
        tixto = sum(self._get_sam_value("SAM", "AG", "GVT", "X", i.upper()) for i in I)
        ...
        tipto = sum(self._get_sam_value("SAM", "AG", "GVT", "J", j.upper()) for j in J)
        ...
        # YGTRO = SUM[agng, TRO('gvt', agng)].
        ygtro = sum(
            self._get_sam_value("SAM", "AG", "GVT", "AG", ag.upper()) for ag in AGNG
        )
    ...
    def _calibrate_rest_of_world(self) -> None:
        ...
        imo_values = {
            i: self._get_sam_value("SAM", "AG", "ROW", "I", i.upper()) for i in I
        }
        ...
        yrowo = (
            ...
            + sum(
                self._get_sam_value("SAM", "AG", "ROW", "AG", ag.upper()) for ag in AG
            )
        )
    ...
    def _calibrate_shares(self) -> None:
        ...
        for ag in AG:
            ...
            for k in K:
                ...
                kdo_sum = sum(
                    self._get_sam_value("SAM", "K", k_upper, "J", j.upper()) for j in J
                )
                if kdo_sum != 0:
