...
*  25% decrease of the indirect tax rates on all commodities  <-- SIM1 shock, kept OFF so
*  this MCP solves the BASE case (to mirror the Pyomo MCP base). Uncomment for the SIM1 slice.
...
* ===== MCP formulation (equation-variable pairing) — PEP-1-1 as a complementarity problem =====
* First PEP-MCP known (PEP official + jasonjensen both use NLP/CNS). 95/97 pairs conform;
* the TR transfer block (EQ47-51 define slices TR(agng,h)/TR('gvt',h)/TR(ag,f)/
* TR(agng,'gvt')/TR(agd,'row')) needs per-slice pairing — see pep_pyomo MCP for the
* programmatic version. e.fx=1 numeraire, WALRAS.LEON free-row.
MODEL PEP11_MCP PEP static MCP /
EQ1.XST, EQ2.CI, EQ3.VA, EQ4.WC, EQ5.LDC, EQ6.LD, EQ7.KDC, EQ8.KD, EQ9.DI, EQ10.YH, EQ11.YHL, EQ12.YHK, EQ13.YHTR, EQ14.YDH, EQ15.CTH, EQ16.SH, EQ17.YF, EQ18.YFK, EQ19.YFTR, EQ20.YDF, EQ21.SF, EQ22.YG, EQ23.YGK, EQ24.TDHT, EQ25.TDFT, EQ26.TPRODN, EQ27.TIWT, EQ28.TIKT, EQ29.TIPT, EQ30.TPRCTS, EQ31.TICT, EQ32.TIMT, EQ33.TIXT, EQ34.YGTR, EQ35.TDH, EQ36.TDF, EQ37.TIW, EQ38.TIK, EQ39.TIP, EQ40.TIC, EQ41.TIM, EQ42.TIX, EQ43.SG, EQ44.YROW, EQ45.SROW, EQ46.CAB, EQ47.TR, EQ48.TR, EQ49.TR, EQ50.TR, EQ51.TR, EQ52.C, EQ53.GFCF, EQ54.INV, EQ55.CG, EQ56.DIT, EQ57.MRGN, EQ58.PT, EQ59.XS, EQ60.EX, EQ61.DS, EQ62.PE_FOB, EQ63.Q, EQ64.IM, EQ65.PP, EQ66.PCI, EQ67.PVA, EQ68.RC, EQ70.WTI, EQ72.RTI, EQ73.R, EQ74.P, EQ75.PL, EQ76.PE, EQ77.PD, EQ78.PM, EQ79.PC, EQ80.PIXGDP, EQ81.PIXCON, EQ82.PIXINV, EQ83.PIXGVT, EQ84.DD, EQ85.W, EQ86.RK, EQ87.IT, EQ88.PD, EQ89.EXD, EQ90.GDP_BP, EQ91.GDP_MP, EQ92.GDP_IB, EQ93.GDP_FD, EQ94.CTH_REAL, EQ95.G_REAL, EQ96.GDP_BP_REAL, EQ97.GDP_MP_REAL, EQ98.GFCF_REAL, WALRAS.LEON
/;

PEP11.HOLDFIXED=1;

* ===== MCP solve (GAMS infers the eq<->var pairing from /ALL/, like `model simple /all/;
* solve using mcp;`) — this is the NLP-vs-NLP-analog reference for the Pyomo MCP: same
* formulation, same PATH engine, so the solver tolerance cancels. =====
OPTION MCP = PATH;
MODEL PEP11_ALLMCP /ALL/;
PEP11_ALLMCP.HOLDFIXED=1;
SOLVE PEP11_ALLMCP USING MCP;

* Unload the SOLVED levels (.L) directly — no sim1/var post-processing, we want the raw
* MCP fixed point to diff against the Pyomo MCP.
* Unload EVERY symbol (all model variables) so the Python seed is complete — a partial
* var list left ~150 free vars at their Python init and fabricated a spurious residual.
execute_unload "Results_mcp_sim1";
