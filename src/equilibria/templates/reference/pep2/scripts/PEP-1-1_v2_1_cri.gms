$TITLE     PEP standard model 1-1, version 2.1 - Modular Version
...
* Sets are loaded dynamically from SAM structure
* If you change aggregation, regenerate with: sam/cri/2016/src/generate_gams_config.py
$include modules/sets_definition.inc

* Alternative: Load from SAM-V2_0.xlsx structure (commented out)
* J All industries
* I All commodities
...
** 1.2 Production factors (already loaded from sets_definition.inc)

* L Labor categories - now loaded dynamically
* /
*  usk             Unskilled workers
*  sk              Skilled workers
* /

* K Capital categories - now loaded dynamically
* /
*  cap             Capital
*  land            Land
* /
...
** 1.3 Agents (now loaded dynamically from sets_definition.inc)

* AG, MARG, and OTH are loaded from SAM structure
* AG includes all agents: households, firms, government, ROW, and tax accounts

SET
* AG All agents - now loaded dynamically from SAM
* /
...
PARAMETERS
 ...
 lambda_WL(ag,l)   Share of type l labor income received by agent ag (includes ROW for cross-border workers)
...
* Check if SAM file is provided as command line parameter
$if not set SAM $set SAM ../data/SAM-V2_0.xlsx

* Display which SAM file is being used
$log === Loading SAM from: %SAM% ===

* Generate GDX from Excel (gamsconnect will handle .gdx files automatically)
* connect.yaml is generated dynamically by Python (read_config.py --generate-connect)
* to handle variable SAM sizes and ensure duplicateRecords setting is present
$call gamsconnect connect.yaml
$GDXIN SAM-V2_0.gdx
$LOAD SAM
$GDXIN

* Manual declaration of data labels (Trimmed)
SET data_labels / 'OTH', 'INV', 'VSTK', 'td', 'ti', 'tm' /;
 ...
 DDO(i)          = SUM[j,DSO(j,i)];
 DIO(i,j)        = SAM('I',i,'J',j);
 EXDO(i)         = SAM('I',i,'Agents','row');
 EXO(j,i)        = 0;
 EXO(j,i)$DDO(i) = DSO(j,i)/DDO(i)*EXDO(i);
 ...
 TDFO(f)         = SAM('Agents','td','Agents',f);
 TDHO(h)         = SAM('Agents','td','Agents',h);
 ...
 lambda_WL(ag,l) = SAM('Agents',ag,'L',l);
...
* Check if VAL_PAR file is provided as command line parameter
$if not set VALPAR $set VALPAR ../data/VAL_PAR.xlsx

* Display which VAL_PAR file is being used
$log === Loading VAL_PAR from: %VALPAR% ===

* Generate GDX from Excel
* connectpar.yaml is generated dynamically by Python (read_config.py --generate-connect)
* to handle variable parameter sizes and ensure duplicateRecords setting is present
...
** 4.1 Calculation of income and savings related variables and parameters
 ...
 YROWO           = SUM[i,IMO(i)]+SUM[k,lambda_RK('row',k)]
                  +SUM[ag,TRO('row',ag)]
                  +SUM[l,lambda_WL('row',l)];   !! CRI: cross-border labor compensation
 CABO            = -SROWO;

 ITO             = SUM[h,SHO(h)]+SUM[f,SFO(f)]+SGO+SROWO;

 lambda_RK(ag,k)$SUM[j,KDO(k,j)] = lambda_RK(ag,k)/SUM[j,KDO(k,j)];
 lambda_WL(ag,l)$SUM[j,LDO(l,j)] = lambda_WL(ag,l)/SUM[j,LDO(l,j)];
 lambda_TR(agng,h)
                 = TRO(agng,h)/YDHO(h);
...
**    5.3.2.4 Rest of the world
 ...
 EQ44..          YROW =e= e*SUM[i$IMO(i),PWM(i)*IM(i)]
                       ...
                       +SUM[agd,TR('row',agd)]
                       +SUM{l,lambda_WL('row',l)*W(l)*SUM[j$LDO(l,j),LD(l,j)]};   !! CRI: cross-border labor
...
* EQ69(j)..       WC(j)*LDC(j) =e= SUM[l$LDO(l,j),WTI(l,j)*LD(l,j)];
 ...
 EQ70(l,j)$LDO(l,j)..     WTI(l,j) =e= W(l)*(1+ttiw(l,j));
...
* Export calibration trade series (EXDO, ttixO, PE_FOBO, PWXO)
$include modules/export_calibration_trade.inc
 RTI.l(k,j)      = RTIO(k,j);
 W.l(l)          = WO(l);
 WC.l(j)         = WCO(j);
 WTI.l(l,j)      = WTIO(l,j);
...
** 6.4 Simulations
*  Simulation scenarios are defined in modules/simulations.inc
*  Usage: gams PEP-1-1_v2_1_modular.gms --scenario=export-tax
$include "modules/simulations.inc"

** 6.5 Resolution
option limrow = 10000;
* TESTING: Changed from conopt3 to IPOPT to test solver hypothesis
* ORIGINAL: OPTION NLP = conopt3; OPTION CNS = conopt4;
* Using IPOPT for both NLP and CNS
OPTION NLP = ipopt;
OPTION CNS = ipopt;
