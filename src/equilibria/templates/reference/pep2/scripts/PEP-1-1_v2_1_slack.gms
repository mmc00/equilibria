...
$STITLE    Single country static version, April 2013
...
PARAMETER
PARJ(j,*), PARI(i,*), PARJI(j,i), PARAG(*,ag);
...
**  5.1.5 Other variables
 ...
 SLACK69(j)      Imbalance in equation 69
 SLACK71(j)      Imbalance in equation 71
 SLACK74a(j)     Imbalance in equation 74a
;
...
* EQ69(j)..       WC(j)*LDC(j) =e= SUM[l$LDO(l,j),WTI(l,j)*LD(l,j)];
 EQ69(j)$LDCO(j)..
                 SLACK69(j) =e= WC(j)*LDC(j)
                           - SUM[l$LDO(l,j),WTI(l,j)*LD(l,j)];
...
*                 RC(j)*KDC(j) =e= SUM[k$KDO(k,j),RTI(k,j)*KD(k,j)];
 EQ71(j)$(kmob and KDCO(j))..
                 SLACK71(j) =e= RC(j)*KDC(j)
                             -SUM[k$KDO(k,j),RTI(k,j)*KD(k,j)];
...
* EQ74(j)..      PT(j)*XST(j) =e= SUM[i,P(j,i)*XS(j,i)];
 EQ74a(j)..      SLACK74a(j)  =e= PT(j)*XST(j)
                              -SUM[i$XSO(j,i),P(j,i)*XS(j,i)];
...
*  50% decrease of the indirect tax rates on all commodities
