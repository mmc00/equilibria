...
* 1 Dynamic set definition (generated from SAM Excel)
$if not set SAM_XLS $setglobal SAM_XLS ../data/SAM-CRI-gams-fixed.xlsx
$if not set DYN_SETS_INC $setglobal DYN_SETS_INC dynamic_sets.inc
$if not set PYTHON_BIN $setglobal PYTHON_BIN python3
$call %PYTHON_BIN% generate_dynamic_sets_inc.py "%SAM_XLS%" "%DYN_SETS_INC%"
$ifE errorLevel<>0 $abort "Failed to generate dynamic sets include from SAM Excel"
$include %DYN_SETS_INC%

* Minimal structural accounts (override via --ACC_*=...)
$if not set ACC_GVT  $setglobal ACC_GVT gvt
$if not set ACC_ROW  $setglobal ACC_ROW row
$if not set ACC_INV  $setglobal ACC_INV INV
$if not set ACC_VSTK $setglobal ACC_VSTK VSTK
$if not set ACC_TD   $setglobal ACC_TD TD
$if not set ACC_TI   $setglobal ACC_TI TI
$if not set ACC_TM   $setglobal ACC_TM TM
$if not set ACC_TX   $setglobal ACC_TX TX
...
* Example:
*   gams PEP-1-1_v2_1_ipopt_excel.gms --SAM_XLS=../data/SAM-V2_0_connect.xlsx --SAM_GDX=../data/SAM-V2_0_from_excel.gdx
* Note: GAMS Connect ExcelReader expects .xlsx
$if not set SAM_XLS $setglobal SAM_XLS ../data/SAM-CRI-gams-fixed.xlsx
$if not set SAM_GDX $setglobal SAM_GDX SAM-CRI-gams-fixed-from-excel.gdx
$onEmbeddedCode Connect:
- ExcelReader:
    file: %SAM_XLS%
    symbols:
      - { name: SAM, rowDimension: 2, columnDimension: 2, range: SAM!A4 }
- GDXWriter:
    file: %SAM_GDX%
    symbols: [ {name: SAM} ]
$offEmbeddedCode
...
$GDXIN
 ...
 CO(i,h)         = SAM('I',i,'AG',h);
 CGO(i)          = SAM('I',i,'AG','%ACC_GVT%');
 DSO(j,i)        = SAM('J',j,'I',i);
 DDO(i)          = SUM[j,DSO(j,i)];
 DIO(i,j)        = SAM('I',i,'J',j);
 EXO(j,i)        = SAM('J',j,'X',i);
 EXDO(i)         = SAM('X',i,'AG','%ACC_ROW%');
 INVO(i)         = SAM('I',i,'OTH','%ACC_INV%');
 VSTKO(i)        = SAM('I',i,'OTH','%ACC_VSTK%');
 IMO(i)          = SAM('AG','%ACC_ROW%','I',i);
 KDO(k,j)        = SAM('K',k,'J',j);
 LDO(l,j)        = SAM('L',l,'J',j);
 SFO(f)          = SAM('OTH','%ACC_INV%','AG',f);
 SGO             = SAM('OTH','%ACC_INV%','AG','%ACC_GVT%');
 SHO(h)          = SAM('OTH','%ACC_INV%','AG',h);
 SROWO           = SAM('OTH','%ACC_INV%','AG','%ACC_ROW%');
 TDFO(f)         = SAM('AG','%ACC_TD%','AG',f);
 TDHO(h)         = SAM('AG','%ACC_TD%','AG',h);
 TICO(i)         = SAM('AG','%ACC_TI%','I',i);
 TIKO(k,j)       = SAM('AG',k,'J',j);
 TIMO(i)         = SAM('AG','%ACC_TM%','I',i);
 TIPO(j)         = SAM('AG','%ACC_GVT%','J',j);
 TIXO(i)         = SAM('AG','%ACC_GVT%','X',i);
...
- ExcelReader:
    file: %VAL_PAR_XLS%
    symbols:
      - { name: PARJ,  range: PAR!A5:E10,  rowDimension: 1, columnDimension: 1 }
      - { name: PARI,  range: PAR!A13:C18, rowDimension: 1, columnDimension: 1 }
      - { name: PARJI, range: PAR!A21:F26, rowDimension: 1, columnDimension: 1 }
      - { name: PARAG, range: PAR!A29:F41, rowDimension: 1, columnDimension: 1 }
- GDXWriter:
    file: %VAL_PAR_GDX%
...
** 4.1 Calculation of income and savings related variables and parameters
 ...
 YDHO(h)         = YHO(h)-TDHO(h)-TRO('%ACC_GVT%',h);
 ...
 YGKO            = SUM[k,lambda_RK('%ACC_GVT%',k)];
 TDHTO           = SUM[h,TDHO(h)];
 TDFTO           = SUM[f,TDFO(f)];
 TICTO           = SUM[i,TICO(i)];
 TIMTO           = SUM[i,TIMO(i)];
 ...
 tr1O(h)         = [TRO('%ACC_GVT%',h)-tr0O(h)]/YHO(h);
...
** 4.4 Calibration of other tax rates, prices, margins and volumes
 ...
 PCO(i)$[DDO(i)+IMO(i)]
                 = [DDO(i)+IMO(i)+SUM(ij,tmrg(ij,i))+TICO(i)+TIMO(i)]
                  /[DDO(i)+IMO(i)];
 PCO(i)$[(not DDO(i)) and (not IMO(i))] = 1;
 ...
 tmrg(i,ij)$[DDO(ij)+IMO(ij)] = tmrg(i,ij)/{DDO(ij)+IMO(ij)};
 tmrg(i,ij)$[(not DDO(ij)) and (not IMO(ij))] = 0;

 tticO(i)$((PLO(i)+SUM[ij,PCO(ij)*tmrg(ij,i)])*DDO(i)
                          +(eO*PWMO(i)+SUM[ij,PCO(ij)*tmrg(ij,i)])*IMO(i)
                          +TIMO(i))
                = TICO(i)/((PLO(i)+SUM[ij,PCO(ij)*tmrg(ij,i)])*DDO(i)
                          +(eO*PWMO(i)+SUM[ij,PCO(ij)*tmrg(ij,i)])*IMO(i)
                          +TIMO(i));
 ...
 PTO(j)$XSTO(j)  = SUM[i$XSO(j,i),PO(j,i)*XSO(j,i)]/XSTO(j);

 QO(i)$PCO(i)    = [PMO(i)*IMO(i)+PDO(i)*DDO(i)]/PCO(i);
 ...
 PCIO(j)$CIO(j)  = SUM[i,PCO(i)*DIO(i,j)]/CIO(j);
 ...
 VAO(j)          = LDCO(j)+KDCO(j);
 PVAO(j)$VAO(j)  = [WCO(j)*LDCO(j)+RCO(j)*KDCO(j)]/VAO(j);

 ttipO(j)$[{PVAO(j)*VAO(j)+SUM[i,PCO(i)*DIO(i,j)]}]
                = TIPO(j)/{PVAO(j)*VAO(j)+SUM[i,PCO(i)*DIO(i,j)]};
 ttipO(j)$[not {PVAO(j)*VAO(j)+SUM[i,PCO(i)*DIO(i,j)]}] = 0;
...
* Keep intercepts/transfers unchanged in dynamic SAM mode.


** 4.6 Calibration of function parameters

**  4.6.1 Leontief functions
 io(j)$XSTO(j)   = CIO(j)/XSTO(j);
 v(j)$XSTO(j)    = VAO(j)/XSTO(j);
 aij(i,j)$CIO(j) = DIO(i,j)/CIO(j);

**  4.6.2 Calibration of CET parameters
**   4.6.2.1 CET between commodities
 rho_XT(j)$sigma_XT(j) = (1+sigma_XT(j))/sigma_XT(j);
 rho_XT(j)$[not sigma_XT(j)] = 1;
 beta_XT(j,i)$XSO(j,i)
                 = PO(j,i)*XSO(j,i)**(1-rho_XT(j))/
                   SUM[ij$XSO(j,ij),PO(j,ij)*XSO(j,ij)**(1-rho_XT(j))];
 B_XT(j)$XSTO(j) = XSTO(j)
...
**   4.6.3.1 Composite good
 rho_M(i)$[IMO(i) and DDO(i) and sigma_M(i)]
                 = (1-sigma_M(i))/sigma_M(i);
 rho_M(i)$[(not IMO(i)) or (not DDO(i)) or (not sigma_M(i))]
                 = -1;
...
**   4.6.3.2 Composite capital
 ...
 rho_KD(j)$[(not KDCO(j)) or (not sigma_KD(j))] = -1;
 beta_KD(k,j)$KDO(k,j)
...
**   4.6.3.3 Composite labor
 rho_LD(j)$sigma_LD(j) = (1-sigma_LD(j))/sigma_LD(j);
 rho_LD(j)$[not sigma_LD(j)] = -1;
 beta_LD(l,j)$LDO(l,j)
...
*   adjusts the elasticities proportionnaly
 ...
 sigma_Y(i,h)$SUM[ij,sigma_Y(ij,h)*PCO(ij)*CO(ij,h)]
                 = sigma_Y(i,h)*CTHO(h)/SUM[ij,sigma_Y(ij,h)*PCO(ij)*CO(ij,h)];
 gamma_LES(i,h)$CTHO(h)  = PCO(i)*CO(i,h)*sigma_Y(i,h)/CTHO(h);
...
EQ14(h)..       YDH(h) =e= YH(h)-TDH(h)-TR('%ACC_GVT%',h);
...
EQ23..          YGK =e= SUM{k,lambda_RK('%ACC_GVT%',k)*SUM[j$KDO(k,j),R(k,j)*KD(k,j)]};
...
EQ34..          YGTR =e= SUM[agng,TR('%ACC_GVT%',agng)];
...
EQ43..          SG =e= YG-SUM[agng,TR(agng,'%ACC_GVT%')]-G;

**    5.3.2.4 Rest of the world

 EQ44..          YROW =e= e*SUM[i$IMO(i),PWM(i)*IM(i)]
                      +SUM{k,lambda_RK('%ACC_ROW%',k)*SUM[j$KDO(k,j),R(k,j)*KD(k,j)]}
                      +SUM[agd,TR('%ACC_ROW%',agd)];

 EQ45..          SROW =e= YROW-SUM[i$EXDO(i),PE_FOB(i)*EXD(i)]
                        -SUM[agd,TR(agd,'%ACC_ROW%')];
...
EQ48(h)..       TR('%ACC_GVT%',h) =e= PIXCON**eta*tr0(h)+tr1(h)*YH(h);

 EQ49(ag,f)..    TR(ag,f) =e= lambda_TR(ag,f)*YDF(f);

EQ50(agng)..    TR(agng,'%ACC_GVT%') =e= PIXCON**eta*TRO(agng,'%ACC_GVT%');

EQ51(agd)..     TR(agd,'%ACC_ROW%') =e= PIXCON**eta*TRO(agd,'%ACC_ROW%');
