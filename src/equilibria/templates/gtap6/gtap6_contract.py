...
def _full_gtap6_equation_ids() -> tuple[str, ...]:
...
class GTAP6ClosureConfig(ModelClosureConfig):
    ...
    pattern as the v7 contract: see :func:`build_gtap6_contract`.
    """

    name: str = "gtap6_standard"
    ...
    fixed: tuple[str, ...] = Field(
        default_factory=lambda: (
            ...
            "tpd",
            "tpi",  # private domestic/import-specific
            "tgd",
            "tgi",  # gov domestic/import-specific
            "tfd",
            "tfi",  # firm domestic/import-specific
    ...
    def _normalize_calibration_source(cls, value: Any) -> str:
        ...
        if text in {"python", "gempack"}:
            return text
        raise ValueError("calibration_source must be 'python' or 'gempack' for v6.2")
...
class GTAP6EquationConfig(ModelEquationConfig):
    """v6.2 equation system selection."""

    name: str = "full_gtap_v62"
    include: tuple[str, ...] = Field(default_factory=_full_gtap6_equation_ids)
    activation_masks: Literal["gtap6_standard", "all_active"] = "gtap6_standard"
...
class GTAP6BoundsConfig(ModelBoundsConfig):
...
class GTAP6Contract(ModelContract):
    """Complete v6.2 contract: closure + equations + bounds."""

    closure: GTAP6ClosureConfig = Field(default_factory=GTAP6ClosureConfig)
    equations: GTAP6EquationConfig = Field(default_factory=GTAP6EquationConfig)
    bounds: GTAP6BoundsConfig = Field(default_factory=GTAP6BoundsConfig)


def _closure_for(name: str) -> GTAP6ClosureConfig:
    """Build a closure config for one of the canonical v6.2 names."""
    canonical = name.strip().lower()
    base_default = GTAP6ClosureConfig()  # snapshot the default fixed/endogenous lists

    if canonical in ("gtap6_standard", "gtap_standard", "standard"):
        return GTAP6ClosureConfig(name="gtap6_standard")
    ...
    if canonical in ("trade_policy", "tariff", "trade"):
        ...
        relaxed_fixed = tuple(
            f for f in base_default.fixed if f not in {"tm", "tms", "tx", "txs"}
        )
        return GTAP6ClosureConfig(
            name="trade_policy",
            fix_taxes=False,
...
def build_gtap6_contract(closure_name: str = "gtap6_standard") -> GTAP6Contract:
    """Build a complete GTAP v6.2 contract.

    Args:
        closure_name: One of ``"gtap6_standard"``, ``"altertax"``,
            ``"trade_policy"``.

    Returns:
        A :class:`GTAP6Contract` with the closure, equations, and
        default bounds wired together.
    """
    closure = _closure_for(closure_name)
    equations = GTAP6EquationConfig()
    return GTAP6Contract(
        name=f"gtap6:{closure_name}",
        closure=closure,
        equations=equations,
    )


def default_gtap6_contract() -> GTAP6Contract:
    """Return the standard v6.2 contract."""
    return build_gtap6_contract("gtap6_standard")
