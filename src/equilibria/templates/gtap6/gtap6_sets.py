...
class GTAP6Sets:
    ...
    def load_from_har(
        ...
        self.i = self._first_set(
            data, _TRAD_COMM_HEADERS, required=True, name="TRAD_COMM"
        )
        self.cgds = (
            self._first_set(data, _CGDS_COMM_HEADERS, required=False, name="CGDS_COMM")
            or []
        )
        self.f = self._first_set(
            data, _ENDW_COMM_HEADERS, required=True, name="ENDW_COMM"
        )
        self.marg = (
            self._first_set(data, _MARG_HEADERS, required=False, name="MARG_COMM") or []
        )
    ...
    def _load_slug(self, default_path: Path) -> None:
        ...
        flat = (
            list(slug_arr.flatten()) if hasattr(slug_arr, "flatten") else list(slug_arr)
        )
    ...
    def __repr__(self) -> str:
        return (
            f"GTAP6Sets({self.aggregation_name}: "
