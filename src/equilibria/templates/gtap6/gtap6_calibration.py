...
The derivations are pure functions of the loaded ``GTAP6Parameters``
benchmark; they don't touch Pyomo at all. The result is a flat
:class:`DerivedGTAP6Calibration` dataclass that the model builder
...
class DerivedGTAP6Calibration:
    ...
    # Trade tax rates
    tms: dict[tuple[str, str, str], float] = field(
        default_factory=dict
    )  # (i, s, r) — tariff
    txs: dict[tuple[str, str, str], float] = field(
        default_factory=dict
    )  # (i, s, r) — export tax
    ...
    vom: dict[tuple[str, str], float] = field(
        default_factory=dict
    )  # (j, r) market price output (output-side)
    voa: dict[tuple[str, str], float] = field(
        default_factory=dict
    )  # (j, r) agent/producer price output
    vop: dict[tuple[str, str], float] = field(
        default_factory=dict
    )  # (j, r) production cost base (= VA + intermediates)
    evom: dict[tuple[str, str], float] = field(
        default_factory=dict
    )  # (f, r) factor income market
    # Value-added aggregate per sector
    va_total: dict[tuple[str, str], float] = field(
        default_factory=dict
    )  # (j, r) total VA at market prices
    ...
    qst_0: dict[tuple[str, str], float] = field(
        default_factory=dict
    )  # margin sales (m, r)
...
def derive_calibration(
    ...
    Returns:
        :class:`DerivedGTAP6Calibration` with all derived tables filled in.
    """
    b = params.benchmark
    out = DerivedGTAP6Calibration()
    ...
    for r in sets.r:
        for i in sets.i:
            out.vim[(i, r)] = sum(b.vims.get((i, s, r), 0.0) for s in sets.r)
            out.vimw[(i, r)] = sum(b.viws.get((i, s, r), 0.0) for s in sets.r)
    ...
    def _armington_calibrate(
        ...
        total_agent = (
            pdom_0 * vdm + pimp_0 * vim
        )  # not vda+via because we include pds_0/pim_0 wedge
    ...
    for r in sets.r:
        ...
        for i in sets.i:
            ...
            ad_h, am_h, pp0 = _armington_calibrate(
                vd_h, vm_h, va_h, vai_h, sigma_d, pds0_i, pim0_i
            )
            ...
            ad_g, am_g, pg0 = _armington_calibrate(
                vd_g, vm_g, va_g, vai_g, sigma_d, pds0_i, pim0_i
            )
        ...
        for i in sets.i:
            ...
            out.qp_0[(i, r)] = b.vdpm.get((i, r), 0.0) + b.vipm.get((i, r), 0.0)
    ...
    for r in sets.r:
        ...
        for i in sets.i:
            ...
            for k in sets.i:
                ...
                out.cde_ep[(i, k, r)] = (out.cde_ape.get((i, k, r), 0.0) - ey_i) * cs_k
            ...
            for s in sets.r:
                for d in sets.r:
                    if s == d:
                        continue
                    total_vtwr = sum(
                        b.vtwr.get((m_other, i, s, d), 0.0) for m_other in sets.marg
                    )
                    if total_vtwr <= 0.0:
                        continue
        ...
        for d in sets.r:
            ...
            for s, qxs0, pms0 in sources_with_flow:
                ...
                if pim0_cost > 0.0 and pms0 > 0.0:
                    out.alpha_xs[(i, s, d)] = (
                        share_s * (pms0 / pim0_cost) ** sigma_m_eff
                    )
                else:
    ...
    for m_lbl in sets.marg:
        ...
        diag_vtwr = sum(
            b.vtwr.get((m_lbl, i, r, r), 0.0) for i in sets.i for r in sets.r
        )
        ...
        if total_vst_full > 0.0:
            for r in sets.r:
                out.share_st[(m_lbl, r)] = b.vst.get((m_lbl, r), 0.0) / total_vst_full
    ...
    for r in sets.r:
        ...
        tout = sum(
            out.vom.get((i, r), 0.0) - out.vop.get((i, r), 0.0) for i in sets.prod_comm
        )
