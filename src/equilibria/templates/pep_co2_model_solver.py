"""PEP-CRI + CO2 solver layer.

Hierarchy:
    PEPCRIIPOPTSolver         (CRI extension — L→ROW in EQ44)
        └── PEPCO2IPOPTSolver     (CO2 extension — carbon tax in EQ39, EQ66)

    PEPCRIModelSolver         (CRI multi-method solver)
        └── PEPCO2ModelSolver     (CO2 multi-method solver)
"""
...
from equilibria.templates.pep_cri_solver import PEPCRIIPOPTSolver, PEPCRIModelSolver
from equilibria.templates.pep_model_equations import PEPModelVariables, SolverResult
from equilibria.templates.pep_model_solver import DEBUG_SIMPLE_ITERATION_METHOD
...
class PEPCO2IPOPTSolver(PEPCRIIPOPTSolver):
    """IPOPT solver for the CRI + CO2 extension.

    Extends PEPCRIIPOPTSolver (which adds L→ROW to EQ44) with
    the CO2 tax wedge in EQ39 and EQ66.
    """
    ...
    def __init__(
        ...
        # Replace CRI equations with the full CO2+CRI equation system
    ...
    def _sync_co2_block(self, vars: PEPModelVariables, *, include_pt: bool) -> None:
        from equilibria.templates.pep_co2_data import carbon_unit_tax as _cut
...
class PEPCO2ModelSolver(PEPCRIModelSolver):
    """Multi-method solver for PEP-CRI + CO2 scenarios."""
