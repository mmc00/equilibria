...
from equilibria.templates.gtap_jax.expr_to_jax import translate, translate_parametric


def build_F(m: Any, vectorize: bool = False):
    ...
    n = len(cons_order)
    per_family = []  # (pos, cell_fn|None, consts_stack, slots_stack) or (pos, None, row_fns)
    for fam, cells in fam_cells.items():
        ...
        if ok:
            ...
            per_family.append((pos, cell_fn, consts_stack, slots_stack))
        else:
            per_family.append((pos, None, [translate(c.body, var_index) for c in cells], None))

    def F(z):
        out = jnp.zeros(n)
        for pos, cell_fn, payload, slots_stack in per_family:
