...
class GTAPFullModel:
    ...
    def _add_parameters(self, model: ConcreteModel) -> None:
        ...
        def create_param(name, indices, values, default):
            if len(indices) == 1:
                setattr(
                    ...
                    name,
                    Param(
                        getattr(model, indices[0]), initialize=values, default=default
                    ),
                )
            elif len(indices) == 2:
                setattr(
        ...
        # Share parameters
        create_param("alpha_va", ["r", "a"], self.shares["alpha_va"], 0.4)
        create_param("alpha_int", ["r", "a", "i"], self.shares["alpha_int"], 0.2)
        create_param("delta_d", ["r", "i"], self.shares["delta_d"], 0.6)
        create_param("delta_m", ["r", "i"], self.shares["delta_m"], 0.4)
        create_param("theta_d", ["r", "i"], self.shares["theta_d"], 0.7)
        create_param("theta_e", ["r", "i"], self.shares["theta_e"], 0.3)
        create_param("alpha_f", ["r", "f", "a"], self.shares["alpha_f"], 0.5)

        # Benchmark values
        create_param("vom", ["r", "a"], self.params.benchmark.vom, 100.0)
    ...
    def _add_variables(self, model: ConcreteModel) -> None:
        ...
        model.yc = Var(
            model.r, within=NonNegativeReals, initialize=60.0, doc="Private expenditure"
        )
        ...
        # Numeraire
        model.pnum = Var(within=NonNegativeReals, initialize=1.0, doc="Price numeraire")

        # Walras check
        model.walras = Var(within=Reals, initialize=0.0, doc="Walras excess demand")
    ...
    def _add_production_block(self, model: ConcreteModel) -> None:
        ...
        def ces_va(model, r, a):
            ...
            if esub == 0:
                ...
                return model.va[r, a] == min(
                    model.xf[r, f, a] / model.alpha_f[r, f, a]
                    for f in model.f
                    if model.alpha_f[r, f, a] > 0
                )
            else:
                # CES
                rho = (esub - 1) / esub if esub != 1 else 0.0
                if rho != 0:
                    return model.va[r, a] ** rho == sum(
                        model.alpha_f[r, f, a] * model.xf[r, f, a] ** rho
                        for f in model.f
                    )
                else:
                    # Cobb-Douglas
                    return log(model.va[r, a]) == sum(
                        model.alpha_f[r, f, a] * log(model.xf[r, f, a]) for f in model.f
                    )
        ...
        def eq_pva(model, r, a):
            return model.pva[r, a] * model.va[r, a] == sum(
                model.pf[r, f, a] * model.xf[r, f, a] for f in model.f
            )
        ...
        def eq_px(model, r, a):
            return (
                model.px[r, a] * model.xp[r, a]
                == model.pva[r, a] * model.va[r, a] + model.pint[r, a] * model.int[r, a]
            )
    ...
    def _add_trade_block(self, model: ConcreteModel) -> None:
        ...
        def eq_xet(model, r, i):
            return model.xet[r, i] == sum(
                model.xe[r, i, rp] for rp in model.rp if rp != r
            )
        ...
        def eq_ps(model, r, i):
            return (
                model.ps[r, i] * model.xs[r, i]
                == model.pd[r, i] * model.xd[r, i] + model.pet[r, i] * model.xet[r, i]
            )
    ...
    def _add_armington_block(self, model: ConcreteModel) -> None:
        ...
        def eq_xmt(model, r, i):
            return model.xmt[r, i] == sum(
                model.xw[r, i, rp] for rp in model.rp if rp != r
            )
        ...
        def eq_pa(model, r, i):
            return (
                model.pa[r, i] * model.xa[r, i]
                == model.pd[r, i] * model.xd[r, i] + model.pmt[r, i] * model.xmt[r, i]
            )
    ...
    def _add_income_block(self, model: ConcreteModel) -> None:
        ...
        def eq_regy(model, r):
            return model.regy[r] == sum(
                model.pf[r, f, a] * model.xf[r, f, a] for f in model.f for a in model.a
            )
    ...
    def _add_market_clearing(self, model: ConcreteModel) -> None:
        ...
        def mkt_goods(model, r, i):
            return model.xa[r, i] == model.xc[r, i] + model.xg[r, i] + model.xi[
                r, i
            ] + sum(model.x[r, a, i] for a in model.a)
