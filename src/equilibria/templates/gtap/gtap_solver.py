...
class GTAPSolver:
    ...
    def __init__(
        ...
        params: GTAPParameters | None = None,
    ):
    ...
    def apply_solution_hint(self, hint: Any) -> int:
        ...
        hint_to_model = {
            ...
            "facty": "facty",
            "ytax_ind": "ytax_ind",
            # Income/capital/tax-aggregate vars (feed eq_facty/eq_ytax_ind/
            # eq_kapEnd/eq_psave/eq_pgdpmp/...); seeded to kill phantom residuals.
            "pi": "pi",
            "kstock": "kstock",
            "kapEnd": "kapEnd",
            "ytaxTot": "ytaxTot",
    ...
    def apply_closure(self, closure: GTAPClosureConfig | None = None) -> None:
        ...
        if closure.fix_endowments:
            if hasattr(self.model, "xft"):
                ...
                for idx in self.model.xft:
                    ...
                    fixed_count += 1
            logger.debug("Fixed factor endowments")
    ...
    def apply_conditional_fixing(self) -> int:
        ...
        if hasattr(self.model, "xe"):
            for idx in self.model.xe:
                val = (
                    self.model.xe[idx].value
                    if self.model.xe[idx].value is not None
                    else 0.0
                )
        ...
        if hasattr(self.model, "xwmg"):
            for idx in self.model.xwmg:
                ...
                r, i, rp = idx[0], idx[1], idx[2]
                tmarg = (
                    float(value(self.model.tmarg[r, i, rp]))
                    if hasattr(self.model, "tmarg")
                    else 0.0
                )
                if tmarg <= 0.0:
        ...
        if hasattr(self.model, "xmgm"):
            for idx in self.model.xmgm:
                ...
                share = (
                    float(value(self.model.amgm[mg, r, i, rp]))
                    if hasattr(self.model, "amgm")
                    else 0.0
                )
                if share <= 0.0:
        ...
        if hasattr(self.model, "p_rai"):
            for idx in self.model.p_rai:
                if len(idx) >= 3:
                    ...
                    if key not in x_rai_flag:
                        val = (
                            self.model.p_rai[idx].value
                            if self.model.p_rai[idx].value is not None
                            else 1.0
                        )
        ...
        if hasattr(self.model, "pp_rai"):
            for idx in self.model.pp_rai:
                if len(idx) >= 3:
                    ...
                    if key not in x_rai_flag:
                        val = (
                            self.model.pp_rai[idx].value
                            if self.model.pp_rai[idx].value is not None
                            else 1.0
                        )
        ...
        if hasattr(self.model, "pp"):
            for idx in self.model.pp:
                val = (
                    self.model.pp[idx].value
                    if self.model.pp[idx].value is not None
                    else 1.0
                )
        ...
        if hasattr(self.model, "pabs"):
            ...
            for idx in self.model.pabs:
                ...
                val = (
                    self.model.pabs[idx].value
                    if self.model.pabs[idx].value is not None
                    else 1.0
                )
        ...
        logger.info(
            f"Conditional fixing applied: {fixed_count} variables fixed based on SAM data"
        )
        return fixed_count
    ...
    def apply_aggressive_fixing_for_mcp(self) -> int:
        ...
        from pyomo.environ import Constraint as Con
        from pyomo.environ import Var
        ...
        logger.info(
            f"MCP gap before aggressive fixing: {gap} (free={free_vars}, cons={constraints})"
        )
        ...
        constraints_data = [
            con_data
            for con_data in self.model.component_data_objects(
        ...
        free_var_data = [
            var_data
            for var_data in self.model.component_data_objects(
        ...
        if constraints_data and free_var_data and current_gap > 0:
            var_id_to_col = {
                id(var_data): col for col, var_data in enumerate(free_var_data)
            }
            ...
            if current_gap <= 0:
                logger.info(
                    "MCP square after structural fixing: %s variables fixed",
                    fixed_count,
                )
                return fixed_count
        ...
        for vname in phase1_vars:
            if current_gap <= 0:
                break
            fixed = fix_var_list(
                [vname], max_to_fix=current_gap, sort_by_abs_value=True
            )
            fixed_count += fixed
            current_gap = get_current_gap()
            if fixed > 0:
        ...
        logger.info(
            f"Aggressive fixing applied: {fixed_count} additional variables fixed"
        )
        logger.info(
            f"MCP gap after aggressive fixing: {final_gap} (free={final_free}, cons={constraints})"
        )
    ...
    def _preflight_path_solver(self, solver: Any) -> None:
        ...
        try:
            ...
            if (
                host_arch == "arm64"
                and "x86_64" in exe_info
                and "arm64" not in exe_info
            ):
                logger.warning(
