...
def _gams_snapshot_from_altertax_gdx(gdx_path: Path, period: str):
    ...
    return GTAPVariableSnapshot(
        ...
        # Income/capital/tax-aggregate vars feeding eq_facty/eq_ytax_ind/
        # eq_kapEnd/eq_psave/eq_pgdpmp etc. GAMS GDX symbol names (camelCase where
        # applicable). _slice yields scalar keys for r-indexed vars automatically.
        pi=_slice(_gv(gdx_path, "pi"), period),
        kstock=_slice(_gv(gdx_path, "kstock"), period),
        kapEnd=_slice(_gv(gdx_path, "kapEnd"), period),
        ytax=_slice(_gv(gdx_path, "ytax"), period),
        ytaxTot=_slice(_gv(gdx_path, "ytaxTot"), period),
        ytaxshr=_slice(_gv(gdx_path, "ytaxshr"), period),
        xcshr=_slice(_gv(gdx_path, "xcshr"), period),
        zcons=_slice(_gv(gdx_path, "zcons"), period),
        nd=_slice(_gv(gdx_path, "nd"), period),
        chif=_slice(_gv(gdx_path, "chif"), period),
        savf=_slice(_gv(gdx_path, "savf"), period),
        psave=_slice(_gv(gdx_path, "psave"), period),
        pgdpmp=_slice(_gv(gdx_path, "pgdpmp"), period),
        pmuv=_slice(_gv(gdx_path, "pmuv"), period),
        pwfact=_slice(_gv(gdx_path, "pwfact"), period),
        rorc=_slice(_gv(gdx_path, "rorc"), period),
        rore=_slice(_gv(gdx_path, "rore"), period),
        rorg=_slice(_gv(gdx_path, "rorg"), period),
        pfy=_slice(_gv(gdx_path, "pfy"), period),
        pm=_slice(_gv(gdx_path, "pm"), period),
        pva=_slice(_gv(gdx_path, "pva"), period),
        pnd=_slice(_gv(gdx_path, "pnd"), period),
