...
def _lift_to_t(init_func_or_value, t_set: Tuple[str, ...]):
    """Wrap a single-period initializer to broadcast across all t in t_set.

    Pyomo passes the model + every index including t. The wrapped fn
    drops t (last positional arg) and calls the original initializer.
    Scalars pass through unchanged.
    """
    if callable(init_func_or_value):
        def _wrapped(m, *idx_with_t):
            idx_no_t = idx_with_t[:-1]
            return init_func_or_value(m, *idx_no_t)
        return _wrapped
    return init_func_or_value
...
class GTAPModelEquations:
    ...
    def _align_xi_xaa_post_scaling(self, model: "ConcreteModel") -> None:
        ...
        delta_xi: dict = {}
        for r in model.r:
            pi_val = max(float(pyo_value(model.pi[r, "base"])), 1e-8)
            xiagg = float(pyo_value(model.xiagg[r, "base"]))
            ...
            for i in model.i:
                ...
                if share <= 0.0:
                    delta_xi[(r, i)] = 0.0
                    continue
                pa_inv = max(float(pyo_value(model.pa[r, i, GTAP_INVESTMENT_AGENT, "base"])), 1e-12)
                xi_old = float(pyo_value(model.xi[r, i, "base"]))
                xi_new = max(share * xiagg * (pi_val / pa_inv) ** sigmai_raw, 0.0)
                delta = xi_new - xi_old
                delta_xi[(r, i)] = delta

                # xi and xaa[inv]
                model.xi[r, i, "base"].set_value(xi_new)
                if hasattr(model, "xaa") and (r, i, GTAP_INVESTMENT_AGENT, "base") in model.xaa:
                    model.xaa[r, i, GTAP_INVESTMENT_AGENT, "base"].set_value(xi_new)

                # Scale xda/xma proportionally so Armington shares are preserved.
                if xi_old > 1e-12 and abs(delta) > 1e-14:
                    k = xi_new / xi_old
                    if hasattr(model, "xda") and (r, i, GTAP_INVESTMENT_AGENT, "base") in model.xda:
                        old = float(pyo_value(model.xda[r, i, GTAP_INVESTMENT_AGENT, "base"]))
                        model.xda[r, i, GTAP_INVESTMENT_AGENT, "base"].set_value(max(old * k, 0.0))
                    if hasattr(model, "xma") and (r, i, GTAP_INVESTMENT_AGENT, "base") in model.xma:
                        old = float(pyo_value(model.xma[r, i, GTAP_INVESTMENT_AGENT, "base"]))
                        model.xma[r, i, GTAP_INVESTMENT_AGENT, "base"].set_value(max(old * k, 0.0))
        ...
        for r in model.r:
            for i in model.i:
                ...
                if hasattr(model, "xd"):
                    total_xd = sum(
                        float(pyo_value(model.xda[r, i, aa, "base"]))
                        / max(float(pyo_value(model.xscale[r, aa])), 1e-12)
                        for aa in model.aa
                    )
                    model.xd[r, i, "base"].set_value(max(total_xd, 1e-8))

                # eq_xmt_agg: xmt = sum_aa(xma/xscale)
                if hasattr(model, "xmt"):
                    total_xmt = sum(
                        float(pyo_value(model.xma[r, i, aa, "base"]))
                        / max(float(pyo_value(model.xscale[r, aa])), 1e-12)
                        for aa in model.aa
                    )
                    model.xmt[r, i, "base"].set_value(max(total_xmt, 1e-8))
                ...
                if hasattr(model, "xds"):
                    model.xds[r, i, "base"].set_value(max(float(pyo_value(model.xd[r, i, "base"])), 1e-8))
                ...
                if omega == float("inf") and hasattr(model, "xet") and hasattr(model, "xs"):
                    xs_val = float(pyo_value(model.xs[r, i, "base"]))
                    xds_val = float(pyo_value(model.xds[r, i, "base"]))
                    xet_old = max(float(pyo_value(model.xet[r, i, "base"])), 1e-12)
                    xet_new = max(xs_val - xds_val, 0.0)
                    if xet_new > 1e-12 and abs(xet_new - xet_old) > 1e-14:
                        model.xet[r, i, "base"].set_value(xet_new)
                        ...
                        if hasattr(model, "xw"):
                            ...
                            if omegaw == float("inf"):
                                for rp in model.rp:
                                    if (r, i, rp, "base") in model.xw:
                                        xw_old = float(pyo_value(model.xw[r, i, rp, "base"]))
                                        if xw_old > 0.0:
                                            model.xw[r, i, rp, "base"].set_value(xw_old * scale)
    ...
    def apply_production_scaling(self, model: "ConcreteModel") -> None:
        ...
        base_pa = {
            (r, i, agent): float(value(model.pa[r, i, agent, "base"]))
            for r in model.r
            for i in model.i
            for agent in final_demand_agents
        }
        base_xaa = {
            (r, i, agent): float(value(model.xaa[r, i, agent, "base"]))
            for r in model.r
            for i in model.i
            for agent in final_demand_agents
            if (r, i, agent, "base") in model.xaa
        }
        base_pefob = {
            (r, i, rp): float(value(model.pefob[r, i, rp, "base"]))
            for r in model.r
            for i in model.i
            for rp in model.rp
        }
        base_pmcif = {
            (rp, i, r): float(value(model.pmcif[rp, i, r, "base"]))
            for rp in model.rp
            for i in model.i
            for r in model.r
        }
        base_xw = {
            (r, i, rp): float(value(model.xw[r, i, rp, "base"]))
            for r in model.r
            for i in model.i
            for rp in model.rp
        }
        ...
        # Scale production aggregates - indexed by (r, a, t)
        for key in model.xp:
            r, a, _t = key
            xscale_val = float(value(model.xscale[r, a]))
            if abs(xscale_val - 1.0) > 1e-12:
        ...
        # xaa is (r,i,aa); xda/xma are now (r,i,aa,t).
        ...
        if hasattr(model, "xd"):
            for r in model.r:
                for i in model.i:
                    total_xd = sum(
                        value(model.xda[r, i, aa, "base"]) / max(value(model.xscale[r, aa]), 1e-12)
                        for aa in model.aa
                    )
                    model.xd[r, i, "base"].set_value(max(total_xd, 1e-8))
        ...
        if hasattr(model, "xds") and hasattr(model, "xda"):
            for r in model.r:
                for i in model.i:
                    ...
                    model.xds[r, i, "base"].set_value(max(total_xds, 1e-8))
            ...
            for r in model.r:
                for i in model.i:
                    ...
                    if not has_export_route:
                        lb = model.xet[r, i, "base"].lb
                        if lb is not None and float(lb) > 0.0:
                            model.xet[r, i, "base"].setlb(0.0)
                        model.xet[r, i, "base"].set_value(0.0)
                    ...
                    numerator = (
                        value(model.ps[r, i, "base"]) * value(model.xs[r, i, "base"])
                        - value(model.pd[r, i, "base"]) * value(model.xds[r, i, "base"])
                    )
                    pet_val = max(value(model.pet[r, i, "base"]), 1e-12)
                    xet_val = max(numerator / pet_val, 0.0)
                    if xet_val <= 0.0:
                        lb = model.xet[r, i, "base"].lb
                        if lb is not None and float(lb) > 0.0:
                            model.xet[r, i, "base"].setlb(0.0)
                    model.xet[r, i, "base"].set_value(xet_val)
                    if hasattr(model, "xet_flag"):
        ...
        if hasattr(model, "gw_share") and hasattr(model, "xw") and hasattr(model, "xet"):
            for r in model.r:
                for i in model.i:
                    xet_val = max(value(model.xet[r, i, "base"]), 1e-12)
                    omegaw = self.params.elasticities.omegaw.get((r, i), float("inf"))
                    for rp in model.rp:
                        if value(model.xw_flag[r, i, rp]) <= 0.0:
                            model.gw_share[r, i, rp].set_value(0.0)
                            continue
                        xw_val = max(value(model.xw[r, i, rp, "base"]), 0.0)
                        pe_val = max(value(model.pe[r, i, rp, "base"]), 1e-12)
                        pet_val = max(value(model.pet[r, i, "base"]), 1e-12)
                        if omegaw == float("inf"):
        ...
        if hasattr(model, "xmt") and hasattr(model, "xma"):
            for r in model.r:
                for i in model.i:
                    total_xm = sum(
                        value(model.xma[r, i, aa, "base"]) / max(value(model.xscale[r, aa]), 1e-12)
                        for aa in model.aa
                    )
                    model.xmt[r, i, "base"].set_value(max(total_xm, 1e-8))
        ...
        if hasattr(model, "xc") and hasattr(model, "c_share") and hasattr(model, "yc"):
            for r in model.r:
                ...
                model.yc[r, "base"].set_value(yc_target)
                for i in model.i:
                    share = max(float(value(model.c_share[r, i]) or 0.0), 0.0)
                    pa_hhd = max(float(value(model.pa[r, i, GTAP_HOUSEHOLD_AGENT, "base"]) or 1.0), 1e-12)
                    if hasattr(model, "xc"):
                        model.xc[r, i, "base"].set_value(max((share * yc_target) / pa_hhd, 0.0))

        if hasattr(model, "xaa") and hasattr(model, "xc"):
            for r in model.r:
                for i in model.i:
                    if (r, i, GTAP_HOUSEHOLD_AGENT, "base") in model.xaa:
                        model.xaa[r, i, GTAP_HOUSEHOLD_AGENT, "base"].set_value(max(value(model.xc[r, i, "base"]), 0.0))
        if hasattr(model, "xaa") and hasattr(model, "xg"):
            for r in model.r:
                for i in model.i:
                    if (r, i, GTAP_GOVERNMENT_AGENT, "base") in model.xaa:
                        model.xaa[r, i, GTAP_GOVERNMENT_AGENT, "base"].set_value(max(value(model.xg[r, i, "base"]), 0.0))
        if hasattr(model, "xaa") and hasattr(model, "xi"):
            for r in model.r:
                for i in model.i:
                    if (r, i, GTAP_INVESTMENT_AGENT, "base") in model.xaa:
                        model.xaa[r, i, GTAP_INVESTMENT_AGENT, "base"].set_value(max(value(model.xi[r, i, "base"]), 0.0))
    ...
    def _refresh_macro_initial_state(self, model: "ConcreteModel") -> None:
        ...
        for r in model.r:
            ...
            if hasattr(model, "kstock") and (r, "base") in model.kstock:
                ...
                if benchmark_kstock > 0.0:
                    model.kstock[r, "base"].set_value(max(benchmark_kstock, 1e-8))

            # ft is zero (rtf wedge already absorbed in production costs since pf*xf=EVFB)
            if hasattr(model, "ytax") and (r, "ft", "base") in model.ytax:
                model.ytax[r, "ft", "base"].set_value(0.0)

            if hasattr(model, "ytax") and (r, "dt", "base") in model.ytax:
                dt_total = 0.0
                for f in model.f:
                    for a in model.a:
            ...
            if hasattr(model, "ytax") and (r, "pt", "base") in model.ytax:
                ...
                for a in model.a:
                    ...
                    for i in outputs:
                        ...
                        if prdtx == 0.0:
                            continue
                        p_rai_v = value(model.p_rai[r, a, i, "base"]) if (r, a, i) in model.p_rai else 1.0
                        x_v = value(model.x[r, a, i, "base"]) if (r, a, i) in model.x else 0.0
                        pt_total += prdtx * p_rai_v * x_v
                model.ytax[r, "pt", "base"].set_value(pt_total)
            ...
            for gy, agents in _ctax_agents.items():
                if not (hasattr(model, "ytax") and (r, gy, "base") in model.ytax):
                    continue
                ctax_total = 0.0
                for aa in agents:
                    for i in model.i:
                        ...
                        if dintx != 0.0 and (r, i, aa, "base") in model.xda:
                            ctax_total += dintx * value(model.pd[r, i, "base"]) * value(model.xda[r, i, aa, "base"]) / max(scale, 1e-12)
                        if mintx != 0.0 and (r, i, aa, "base") in model.xma:
                            ctax_total += mintx * value(model.pmt[r, i, "base"]) * value(model.xma[r, i, aa, "base"]) / max(scale, 1e-12)
                model.ytax[r, gy, "base"].set_value(ctax_total)

            if hasattr(model, "facty") and (r, "base") in model.facty:
                gross_factor_income = sum(
                    value(model.pf[r, f, a, "base"]) * value(model.xf[r, f, a, "base"]) / max(value(model.xscale[r, a]), 1e-12)
                    for f in model.f
                    for a in model.a
                )
                model.facty[r, "base"].set_value(
            ...
            if hasattr(model, "regy") and (r, "base") in model.regy:
                ...
                if not model.regy[r, "base"].fixed:
                    model.regy[r, "base"].set_value(value(model.facty[r, "base"]) + value(model.ytax_ind[r, "base"]))
            regy_raw = value(model.regy[r, "base"])
            regy_val = max(abs(regy_raw), 1e-8)
            if hasattr(model, "ytaxshr"):
                for gy in model.gy:
                    if (r, gy, "base") in model.ytaxshr:
                        model.ytaxshr[r, gy, "base"].set_value(value(model.ytax[r, gy, "base"]) / regy_val)
            if hasattr(model, "yc") and (r, "base") in model.yc:
                # GAMS benchmark identity (cal.gms): yc = sum_i pa(r,i,hhd) * xa(r,i,hhd)
                yc_demand = sum(
                    value(model.pa[r, i, GTAP_HOUSEHOLD_AGENT, "base"]) * value(model.xaa[r, i, GTAP_HOUSEHOLD_AGENT, "base"])
                    for i in model.i
                    if (r, i, GTAP_HOUSEHOLD_AGENT, "base") in model.pa and (r, i, GTAP_HOUSEHOLD_AGENT, "base") in model.xaa
                )
                if yc_demand > 0.0:
                    model.yc[r, "base"].set_value(yc_demand)
                else:
                    model.yc[r, "base"].set_value(
                        value(model.betap[r]) * (value(model.phi[r, "base"]) / max(value(model.phip[r, "base"]), 1e-8)) * regy_raw
                    )
            if hasattr(model, "yg") and (r, "base") in model.yg:
                # GAMS benchmark identity (cal.gms): yg = sum_i pa(r,i,gov) * xa(r,i,gov)
                yg_demand = sum(
                    value(model.pa[r, i, GTAP_GOVERNMENT_AGENT, "base"]) * value(model.xaa[r, i, GTAP_GOVERNMENT_AGENT, "base"])
                    for i in model.i
                    if (r, i, GTAP_GOVERNMENT_AGENT, "base") in model.pa and (r, i, GTAP_GOVERNMENT_AGENT, "base") in model.xaa
                )
                if yg_demand > 0.0:
                    model.yg[r, "base"].set_value(yg_demand)
                else:
                    model.yg[r, "base"].set_value(value(model.betag[r]) * value(model.phi[r]) * regy_raw)
            if hasattr(model, "rsav") and (r, "base") in model.rsav:
                ...
                model.rsav[r, "base"].set_value(save_bench)
            ...
            if hasattr(model, "aus") and r in model.aus:
                rsav_val = value(model.rsav[r, "base"]) if hasattr(model, "rsav") and (r, "base") in model.rsav else 0.0
                pop_val = value(model.pop[r])
                psave_val = (
                    value(model.psave[r, "base"])
                    if hasattr(model, "psave") and (r, "base") in model.psave
                    else 1.0
                )
            ...
            if hasattr(model, "chif") and (r, "base") in model.chif:
                model.chif[r, "base"].set_value(value(model.savf[r, "base"]) / regy_val)
            if hasattr(model, "yi") and (r, "base") in model.yi:
                ...
                model.yi[r, "base"].set_value(
                    value(model.pi[r, "base"]) * value(model.depr[r]) * value(model.kstock[r, "base"])
                    + value(model.rsav[r, "base"])
                    + value(model.savf[r, "base"])
                )
            if hasattr(model, "us") and (r, "base") in model.us:
                model.us[r, "base"].set_value(
                    value(model.aus[r]) * value(model.rsav[r, "base"])
                    / max(value(model.psave[r, "base"]) * value(model.pop[r]), 1e-8)
                )
            ...
            if hasattr(model, "pmt"):
                for i in model.i:
                    ...
                    if terms:
                        rhs = sum(terms)
                        if rhs > 0.0:
                            model.pmt[r, i, "base"].set_value(max(rhs ** (1.0 / expo), 1e-8))
            ...
            for i in model.i:
                for agent in final_demand_agents:
                    pa_t = float(value(model.pa[r, i, agent, "base"]))
                    xa_t = float(value(model.xaa[r, i, agent, "base"])) if (r, i, agent, "base") in model.xaa else 0.0
                    pa_0 = base_pa[(r, i, agent)]
                    xa_0 = base_xaa.get((r, i, agent), 0.0)
                    mqabs_tt += pa_t * xa_t
                    mqabs_t0 += pa_t * xa_0
                    mqabs_0t += pa_0 * xa_t
                    mqabs_00 += pa_0 * xa_0
            ...
            for i in model.i:
                for rp in model.rp:
                    pexp_t = float(value(model.pefob[r, i, rp, "base"]))
                    pexp_0 = base_pefob[(r, i, rp)]
                    xexp_t = float(value(model.xw[r, i, rp, "base"]))
                    xexp_0 = base_xw[(r, i, rp)]
                    pimp_t = float(value(model.pmcif[rp, i, r, "base"]))
                    pimp_0 = base_pmcif[(rp, i, r)]
                    ximp_t = float(value(model.xw[rp, i, r, "base"]))
            ...
            model.kapEnd[r, "base"].set_value(
                max((1.0 - value(model.depr[r])) * value(model.kstock[r, "base"]) + value(model.xiagg[r, "base"]), 1e-8)
            )
            ...
            arent_val = cap_return / max(value(model.kstock[r, "base"]), 1e-8) if capital_factors else 0.0
            model.arent[r, "base"].set_value(max(arent_val, 1e-8))
            rorc_val = arent_val / pi_val - value(model.fdepr[r])
            model.rorc[r, "base"].set_value(rorc_val)
            rore_val = rorc_val * (value(model.kstock[r, "base"]) / max(value(model.kapEnd[r, "base"]), 1e-8)) ** value(model.rorflex[r])
            model.rore[r, "base"].set_value(rore_val)

        xigbl_current = sum(
            value(model.xiagg[r, "base"]) - value(model.depr[r]) * value(model.kstock[r, "base"])
            for r in model.r
        )
        model.xigbl["base"].set_value(max(xigbl_current, 1e-8))
        pigbl_numer = sum(
            value(model.pi[r, "base"]) * (value(model.xiagg[r, "base"]) - value(model.depr[r]) * value(model.kstock[r, "base"]))
            for r in model.r
        )
        model.pigbl["base"].set_value(max(pigbl_numer / max(value(model.xigbl["base"]), 1e-8), 1e-8))
        rorg_numer = sum(
            value(model.rore[r, "base"]) * value(model.pi[r, "base"]) * (value(model.xiagg[r, "base"]) - value(model.depr[r]) * value(model.kstock[r, "base"]))
            for r in model.r
        )
        model.rorg["base"].set_value(rorg_numer / max(pigbl_numer, 1e-8))
        residual_gap = sum(
            value(model.yi[r, "base"]) - (value(model.pi[r, "base"]) * value(model.depr[r]) * value(model.kstock[r, "base"]) + value(model.rsav[r, "base"]) + value(model.savf[r, "base"]))
            for r in model.r
    ...
    def _refresh_cet_share_state(self, model: "ConcreteModel") -> None:
        ...
        for r in model.r:
            for i in model.i:
                xs_val = max(value(model.xs[r, i, "base"]), 1e-12)
                xds_val = max(value(model.xds[r, i, "base"]), 0.0)
                xet_val = max(value(model.xet[r, i, "base"]), 0.0)
                pd_val = max(value(model.pd[r, i, "base"]), 1e-12)
                ps_val = max(value(model.ps[r, i, "base"]), 1e-12)
                pet_val = max(value(model.pet[r, i, "base"]), 1e-12)
                omega = self.params.elasticities.omegax.get((r, i), float("inf"))
    ...
    def _add_parameters(self, model: "ConcreteModel") -> None:
        ...
        if self.t0_snapshot is not None:
            ...
            t0 = self.t0_snapshot
            for region in self.sets.r:
                xiagg_v = float(_val(t0.xiagg[region, "base"]))
                pi_v = float(_val(t0.pi[region, "base"])) if hasattr(t0, "pi") else 1.0
                sigmai = float(self.params.elasticities.esubi.get(region, 0.0))
                if abs(sigmai - 1.0) < 1e-8:
                    sigmai = 1.01
                ...
                if xiagg_v > 1e-12 and pi_v > 1e-12:
                    for i in self.sets.i:
                        if (region, i, GTAP_INVESTMENT_AGENT) in t0.xaa and (region, i, GTAP_INVESTMENT_AGENT, "base") in t0.pa:
                            xa_v = float(_val(t0.xaa[region, i, GTAP_INVESTMENT_AGENT]))
                            pa_v = float(_val(t0.pa[region, i, GTAP_INVESTMENT_AGENT, "base"]))
                ...
                if hasattr(t0, "xg") and hasattr(t0, "pg"):
                    ...
                    if xg_v > 1e-12 and pg_v > 1e-12:
                        for i in self.sets.i:
                            if (region, i, GTAP_GOVERNMENT_AGENT) in t0.xaa and (region, i, GTAP_GOVERNMENT_AGENT, "base") in t0.pa:
                                xa_v = float(_val(t0.xaa[region, i, GTAP_GOVERNMENT_AGENT]))
                                pa_v = float(_val(t0.pa[region, i, GTAP_GOVERNMENT_AGENT, "base"]))
                                if xa_v > 0.0 and pa_v > 0.0:
    ...
    def _add_variables(self, model: "ConcreteModel") -> None:
        ...
        def get_xet_init(m, r, i):
            ...
            numerator = value(model.ps[r, i, "base"]) * xs_bench - value(model.pd[r, i, "base"]) * xd_bench
            pet_val = 1.0
            if self.reference_snapshot:
        ...
        model.xp = Var(model.r, model.a, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_vom_init, self._t_set), doc="Production")
        model.x = Var(model.r, model.a, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_make_init, self._t_set), doc="Output")
        model.px = Var(model.r, model.a, model.t, within=NonNegativeReals, initialize=1.0, doc="Unit cost")
        model.p_rai = Var(model.r, model.a, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_p_rai_init, self._t_set), doc="Pre-tax producer price by activity-commodity")
        model.pp = Var(model.r, model.a, model.t, within=NonNegativeReals, initialize=1.0, doc="Producer price (activity aggregate)")
        model.pp_rai = Var(model.r, model.a, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_pp_rai_init, self._t_set), doc="Producer price by activity-commodity")

        # Fix cross-activity-commodity p_rai and pp_rai at build time.
        # eq_x / eq_pp_rai only generate when i ∈ activity_commodities[a]. For
        # all (r,a,i,t) where i ∉ outputs(a) the vars have NO defining equation
        # and must be pinned so the structural bipartite matching stays square.
        # apply_aggressive_fixing_for_mcp handles this at base (gap>0 path) but
        # not at check/shock (gap<=0). Fix upfront to be robust across all periods.
        for _a in self.sets.a:
            _outputs = set(self.sets.activity_commodities.get(_a, []))
            for _r in self.sets.r:
                for _i in self.sets.i:
                    if _i in _outputs:
                        continue
                    for _t in self._t_set:
                        if (_r, _a, _i, _t) in model.p_rai:
                            _vd = model.p_rai[_r, _a, _i, _t]
                            if not _vd.fixed:
                                _vd.fix(float(_vd.value) if _vd.value is not None else 1.0)
                        if (_r, _a, _i, _t) in model.pp_rai:
                            _vd = model.pp_rai[_r, _a, _i, _t]
                            if not _vd.fixed:
                                _vd.fix(float(_vd.value) if _vd.value is not None else 1.0)

        # Supply (3 vars per r,i)
        model.xs = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xs_init, self._t_set), doc="Domestic supply")
        model.xds = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xds_init, self._t_set), doc="Supply of domestically produced goods")
        model.ps = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=1.0, doc="Supply price")
        model.pd = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=1.0, doc="Domestic price")
        
        # GAMS: pa(r,i,aa,t) - Agent-specific Armington price
        def get_pa_init(m, r, i, aa):
            return get_pa_benchmark_init(m, r, i, aa)

        model.pa = Var(model.r, model.i, model.aa, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_pa_init, self._t_set), doc="Armington price by agent")
        ...
        model.dintx = Var(
            ...
            within=Reals,
            bounds=(-0.999, None),
            initialize=_lift_to_t(get_dintx_init, self._t_set),
        ...
        model.mintx = Var(
            ...
            within=Reals,
            bounds=(-0.999, None),
            initialize=_lift_to_t(get_mintx_init, self._t_set),
        ...
        model.xd = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xd_init, self._t_set), doc="Domestic demand")
        model.xmt = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xmt_init, self._t_set), doc="Import demand")
        model.pmt = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_pmt_init, self._t_set), doc="Import price")
        model.xda = Var(model.r, model.i, model.aa, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xda_init, self._t_set), doc="Domestic Armington demand by agent")
        model.xma = Var(model.r, model.i, model.aa, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xma_init, self._t_set), doc="Imported Armington demand by agent")
        ...
        # paa is now just an alias to pa[r,i,aa,t]
        def paa_expr_rule(m, r, i, aa, t):
            return m.pa[r, i, aa, t]
        model.paa = Expression(model.r, model.i, model.aa, model.t, rule=paa_expr_rule, doc="Agent Armington price (expression alias)")

        def pdp_expr_rule(m, r, i, aa, t):
            # GAMS: pdp = pd * (1 + dintx)
            return (1.0 + m.dintx[r, i, aa, t]) * m.pd[r, i, t]
        model.pdp = Expression(model.r, model.i, model.aa, model.t, rule=pdp_expr_rule, doc="Agent domestic demand price (expression)")

        def pmp_expr_rule(m, r, i, aa, t):
            # GAMS: pmp = pmt * (1 + mintx)
            return (1.0 + m.mintx[r, i, aa, t]) * m.pmt[r, i, t]
        model.pmp = Expression(model.r, model.i, model.aa, model.t, rule=pmp_expr_rule, doc="Agent import demand price (expression)")
        
        # Trade - Domestic/Export split (4 vars per r,i)
        model.xet = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xet_init, self._t_set), doc="Export supply")
        ...
        model.pet = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_pet_init, self._t_set), doc="Export price")

        # Value added/intermediate bundles
        model.va = Var(model.r, model.a, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_va_init, self._t_set), doc="Value added bundle")
        model.nd = Var(model.r, model.a, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_nd_init, self._t_set), doc="Intermediate bundle")
        model.pva = Var(model.r, model.a, model.t, within=NonNegativeReals, initialize=1.0, doc="Value added price")
        model.pnd = Var(model.r, model.a, model.t, within=NonNegativeReals, initialize=1.0, doc="Intermediate price")
        ...
        model.xe = Var(model.r, model.i, model.rp, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xe_init, self._t_set), doc="Bilateral exports")
        model.xw = Var(model.r, model.i, model.rp, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xw_init, self._t_set), doc="Bilateral imports")
        model.pe = Var(
            model.r,
            model.i,
            model.rp,
            ...
            initialize=_lift_to_t(get_pe_init, self._t_set),
        ...
        model.pwmg = Var(
            ...
            model.t,
            within=NonNegativeReals,
            initialize=_lift_to_t(get_pwmg_init, self._t_set),
        ...
        model.xwmg = Var(
            ...
            model.t,
            within=NonNegativeReals,
            initialize=_lift_to_t(get_xwmg_init, self._t_set),
        ...
        model.xmgm = Var(
            ...
            model.t,
            within=NonNegativeReals,
            initialize=_lift_to_t(get_xmgm_init, self._t_set),
        ...
        model.xtmg = Var(
            ...
            model.t,
            within=NonNegativeReals,
            initialize=lambda m, margin, t: get_xtmg_init(m, margin),
        ...
        for r in self.sets.r:
            for i in self.sets.i:
                for t in self._t_set:
                    model.etax[r, i, t].fix(0.0)
                    model.mtax[r, i, t].fix(0.0)
        for rp in self.sets.r:
            for i in self.sets.i:
                for r in self.sets.r:
                    for t in self._t_set:
                        model.lambdam[rp, i, r, t].fix(1.0)
        ...
        model.afecom = Var(model.f, model.t, within=Reals, initialize=0.0, doc="World-wide tech shift in VA demand by factor")
        model.afesec = Var(model.a, model.t, within=Reals, initialize=0.0, doc="World-wide tech shift in VA demand by activity")
        model.afefac = Var(model.r, model.f, model.t, within=Reals, initialize=0.0, doc="Region-wide tech shift in VA demand across factors")
        model.afereg = Var(model.r, model.t, within=Reals, initialize=0.0, doc="Region-wide tech shift in VA demand")
        model.afeall = Var(model.r, model.f, model.a, model.t, within=Reals, initialize=0.0, doc="Region/factor/activity tech shift in VA demand")
        model.aiocom = Var(model.i, model.t, within=Reals, initialize=0.0, doc="World-wide tech shift in IO demand by input")
        model.aiosec = Var(model.a, model.t, within=Reals, initialize=0.0, doc="World-wide tech shift in IO demand by activity")
        model.aioreg = Var(model.r, model.t, within=Reals, initialize=0.0, doc="Region-wide tech shift in IO demand")
        model.aioall = Var(model.r, model.i, model.a, model.t, within=Reals, initialize=0.0, doc="Region/input/activity tech shift in IO demand")
        model.andsec = Var(model.a, model.t, within=Reals, initialize=0.0, doc="World-wide tech shift in ND demand by sector")
        model.andreg = Var(model.r, model.t, within=Reals, initialize=0.0, doc="Region-wide tech shift in ND demand")
        model.andall = Var(model.r, model.a, model.t, within=Reals, initialize=0.0, doc="Region/sector tech shift in ND demand")
        model.avasec = Var(model.a, model.t, within=Reals, initialize=0.0, doc="World-wide tech shift in VA demand by sector")
        model.avareg = Var(model.r, model.t, within=Reals, initialize=0.0, doc="Region-wide tech shift in VA demand")
        model.avaall = Var(model.r, model.a, model.t, within=Reals, initialize=0.0, doc="Region/sector tech shift in VA demand")

        for f in self.sets.f:
            for t in self._t_set:
                model.afecom[f, t].fix(0.0)
            for r in self.sets.r:
                for t in self._t_set:
                    model.afefac[r, f, t].fix(0.0)
        for a in self.sets.a:
            for t in self._t_set:
                model.afesec[a, t].fix(0.0)
                model.aiosec[a, t].fix(0.0)
                model.andsec[a, t].fix(0.0)
                model.avasec[a, t].fix(0.0)
        for r in self.sets.r:
            for t in self._t_set:
                model.afereg[r, t].fix(0.0)
                model.aioreg[r, t].fix(0.0)
                model.andreg[r, t].fix(0.0)
                model.avareg[r, t].fix(0.0)
        for i in self.sets.i:
            for t in self._t_set:
                model.aiocom[i, t].fix(0.0)
        for r in self.sets.r:
            for f in self.sets.f:
                for a in self.sets.a:
                    for t in self._t_set:
                        model.afeall[r, f, a, t].fix(0.0)
            for i in self.sets.i:
                for a in self.sets.a:
                    for t in self._t_set:
                        model.aioall[r, i, a, t].fix(0.0)
            for a in self.sets.a:
                for t in self._t_set:
                    model.andall[r, a, t].fix(0.0)
                    model.avaall[r, a, t].fix(0.0)

        # Tier A — lambda/axp shifters (cal.gms inits to 1; recurrence keeps =1 with shifters=0).
        model.lambdaf = Var(model.r, model.f, model.a, model.t, within=NonNegativeReals, initialize=1.0, doc="Factor specific technical change")
        model.lambdai = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=1.0, doc="Investment expenditure technology")
        model.lambdand = Var(model.r, model.a, model.t, within=NonNegativeReals, initialize=1.0, doc="ND bundle shifter")
        model.lambdava = Var(model.r, model.a, model.t, within=NonNegativeReals, initialize=1.0, doc="VA bundle shifter")
        model.axp = Var(model.r, model.a, model.t, within=NonNegativeReals, initialize=1.0, doc="Production frontier shifter")
        model.axpsec = Var(model.a, model.t, within=Reals, initialize=0.0, doc="World-wide shift in production by sector")
        model.axpreg = Var(model.r, model.t, within=Reals, initialize=0.0, doc="Region-wide shift in production")
        model.axpall = Var(model.r, model.a, model.t, within=Reals, initialize=0.0, doc="Region/sector shift in production")
        # kappaf(r,f,a) = (EVFB - EVOS) / EVFB per cal.gms:143-146. Load EVFB/EVOS direct from GDX.
        model.kappaf = Var(model.r, model.f, model.a, model.t, within=Reals, initialize=0.0, doc="Income tax on factor f used in activity a")
        kappaf_init = self._compute_kappaf_init() if hasattr(self, "_compute_kappaf_init") else {}

        for a in self.sets.a:
            for t in self._t_set:
                model.axpsec[a, t].fix(0.0)
        for r in self.sets.r:
            for t in self._t_set:
                model.axpreg[r, t].fix(0.0)
            for i in self.sets.i:
                for t in self._t_set:
                    model.lambdai[r, i, t].fix(1.0)
            for a in self.sets.a:
                for t in self._t_set:
                    model.axp[r, a, t].fix(1.0)
                    model.axpall[r, a, t].fix(0.0)
                    model.lambdand[r, a, t].fix(1.0)
                    model.lambdava[r, a, t].fix(1.0)
                for f in self.sets.f:
                    kf_val = float(kappaf_init.get((r, f, a), 0.0) or 0.0)
                    for t in self._t_set:
                        model.lambdaf[r, f, a, t].fix(1.0)
                        model.kappaf[r, f, a, t].fix(kf_val)

        # Tier C — tax shifters (=0 base, used to redistribute tax burden in scenarios).
        model.dtxshft = Var(model.r, model.i, model.aa, model.t, within=Reals, initialize=0.0, doc="Domestic indirect tax shifter")
        model.mtxshft = Var(model.r, model.i, model.aa, model.t, within=Reals, initialize=0.0, doc="Imported indirect tax shifter")
        model.rtxshft = Var(model.r, model.aa, model.t, within=Reals, initialize=0.0, doc="Uniform indirect tax shifter")

        for r in self.sets.r:
            for aa in model.aa:
                for t in self._t_set:
                    model.rtxshft[r, aa, t].fix(0.0)
                for i in self.sets.i:
                    for t in self._t_set:
                        model.dtxshft[r, i, aa, t].fix(0.0)
                        model.mtxshft[r, i, aa, t].fix(0.0)
        ...
        def _trade_lambdam_value(exporter, commodity, importer) -> float:
            return max(_safe_trade_component_value("lambdam", (exporter, commodity, importer, "base"), 1.0), 1e-12)
        ...
        def _trade_mtax_value(importer, commodity, exporter) -> float:
            ...
            return _safe_trade_component_value("mtax", (importer, commodity, "base"), 0.0)
        ...
        def _trade_etax_value(exporter, commodity, importer) -> float:
            ...
            return _safe_trade_component_value("etax", (exporter, commodity, "base"), 0.0)
        ...
        model.pm = Var(
            ...
            model.t,
            within=NonNegativeReals,
            initialize=_lift_to_t(get_pm_init, self._t_set),
        ...
        model.pmcif = Var(
            ...
            model.t,
            within=NonNegativeReals,
            initialize=_lift_to_t(get_pmcif_init, self._t_set),
        ...
        model.pefob = Var(
            ...
            model.t,
            within=NonNegativeReals,
            initialize=_lift_to_t(get_pefob_init, self._t_set),
        ...
        model.pfy = Var(model.r, model.f, model.a, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_pfy_init, self._t_set), doc="After-tax factor price")
        ...
        model.xc = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xc_init, self._t_set), doc="Private consumption")
        model.xg = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_vgm_init, self._t_set), doc="Government consumption")
        model.xi = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xi_init, self._t_set), doc="Investment")
        model.xaa = Var(model.r, model.i, model.aa, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xaa_init, self._t_set), doc="Agent/activity Armington demand")

        # xa(r,i) is an Expression alias of sum_aa(xaa/xscale) + vst,
        # mirroring GAMS which has no aggregate xa(r,i) variable.
        # Anchored to base period (used in equilibrium initialization only).
        def _xa_expr_rule(m, r, i):
            absorption = sum(m.xaa[r, i, aa, "base"] / m.xscale[r, aa] for aa in m.aa if (r, i, aa, "base") in m.xaa)
        ...
        model.regy = Var(model.r, model.t, within=Reals, initialize=_lift_to_t(get_regy_init, self._t_set), doc="Regional income")
        ...
        # rebase. Mirror this: fix regy[r,'base'] = expenditure_bench at base only.
        if not self.is_counterfactual:
            for r_ in self.sets.r:
                ...
                if expenditure_bench > 0.0:
                    model.regy[r_, "base"].set_value(expenditure_bench)
                    model.regy[r_, "base"].fix(expenditure_bench)
        model.yc = Var(
            model.r,
            model.t,
            within=Reals,
            initialize=_lift_to_t(
                lambda m, r: float(self.reference_snapshot.yc.get(r)) if self.reference_snapshot and self.reference_snapshot.yc.get(r) is not None else get_benchmark_yc(r),
                self._t_set,
            ),
            doc="Private income",
        )
        model.yg = Var(
            ...
            within=Reals,
            initialize=_lift_to_t(
                lambda m, r: float(self.reference_snapshot.yg.get(r)) if self.reference_snapshot and self.reference_snapshot.yg.get(r) is not None else get_benchmark_yg(r),
                self._t_set,
            ),
        ...
        model.yi = Var(
            ...
            model.t,
            within=Reals,
            initialize=_lift_to_t(get_yi_init, self._t_set),
        ...
        model.ytaxshr = Var(
            ...
            within=Reals,
            initialize=_lift_to_t(
                lambda m, r, gy: float(get_ytax_stream_init(m, r, gy)) / max(float(get_regy_init(m, r)), 1e-12),
                self._t_set,
            ),
        ...
        model.kapEnd = Var(model.r, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_kapend_init, self._t_set), doc="End-of-period capital stock")
        model.arent = Var(model.r, model.t, within=NonNegativeReals, initialize=0.05, doc="Rate of return after tax")
        model.rorc = Var(model.r, model.t, within=Reals, initialize=0.05, doc="Net rate of return to capital")
        model.rore = Var(model.r, model.t, within=Reals, initialize=0.05, doc="Expected rate of return")
        ...
        model.pcons = Var(model.r, model.t, within=NonNegativeReals, initialize=1.0, doc="Consumer price index")
        model.xcshr = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xcshr_init, self._t_set), doc="Household budget share")
        ...
        model.zcons = Var(model.r, model.i, model.t, within=NonNegativeReals, initialize=_lift_to_t(get_zcons_init, self._t_set), doc="CDE auxiliary share factor")
        # phip becomes a variable under CDE; initialized from calibration.
        model.phip = Var(model.r, model.t, within=NonNegativeReals, initialize=_lift_to_t(lambda m, r: float(m.phip0[r]), self._t_set), doc="Elasticity of expenditure wrt private utility")
        # phi: GAMS phieq closes betaP/phiP + betaG + betaS to sum to 1/phi.
        model.phi = Var(model.r, model.t, within=NonNegativeReals, initialize=_lift_to_t(lambda m, r: float(m.phi0[r]), self._t_set), doc="Elasticity of total expenditure wrt utility")
        model.uh = Var(model.r, model.t, within=NonNegativeReals, initialize=1.0, doc="Private utility per capita")
        model.ug = Var(model.r, model.t, within=NonNegativeReals, initialize=1.0, doc="Government utility per capita")
        model.us = Var(model.r, model.t, within=NonNegativeReals, initialize=1.0, doc="Savings utility per capita")
        model.u = Var(model.r, model.t, within=NonNegativeReals, initialize=1.0, doc="Total utility")
        model.psave = Var(model.r, model.t, within=NonNegativeReals, initialize=1.0, doc="Price of savings")
        model.savf = Var(model.r, model.t, within=Reals, initialize=_lift_to_t(get_savf_init, self._t_set), doc="Foreign savings")
        model.chif = Var(
            model.r,
            model.t,
            within=Reals,
            initialize=_lift_to_t(lambda m, r: float(m.chif0[r]), self._t_set),
        ...
        model.ev = Var(
            ...
            within=NonNegativeReals,
            initialize=_lift_to_t(
                lambda m, r: max(float(m.yc[r, "base"].value if m.yc[r, "base"].value is not None else 1.0), 1e-8),
                self._t_set,
            ),
        ...
        model.xigbl = Var(model.t, within=NonNegativeReals, initialize=_lift_to_t(get_xigbl_init, self._t_set), doc="Global net investment")
        model.pigbl = Var(model.t, within=NonNegativeReals, initialize=_lift_to_t(get_pigbl_init, self._t_set), doc="Price of global investment")
        model.chiSave = Var(model.t, within=NonNegativeReals, initialize=1.0, doc="Savings price adjustment")
        model.rorg = Var(model.t, within=NonNegativeReals, initialize=1.0, doc="Global rate of return")
        ...
        for r in self.sets.r:
            ...
            for i in self.sets.i:
                try:
                    xv = float(_val_xaflag(model.xaa[r, i, "hhd", "base"]))
                except Exception:
                    xv = 0.0
                xa_r[i] = xv
                denom += xv
    ...
    def _add_equations(self, model: "ConcreteModel") -> None:
        ...
        def _lambdam_value(exporter, commodity, importer) -> float:
            return max(_safe_component_value("lambdam", (exporter, commodity, importer, "base"), 1.0), 1e-12)
        ...
        def _mtax_value(importer, commodity, exporter) -> float:
            ...
            return _safe_component_value("mtax", (importer, commodity, "base"), 0.0)

        def _etax_value(exporter, commodity, importer) -> float:
            # GAMS etax(r,i) is indexed by exporter and commodity only.
            return _safe_component_value("etax", (exporter, commodity, "base"), 0.0)
        ...
        # --------------------------------------------------------------------
        def _m_pp(region, activity, commodity, t="base"):
            if if_sub:
                return (1.0 + value(model.prdtx_rai[region, activity, commodity])) * model.p_rai[region, activity, commodity, t]
            return model.pp_rai[region, activity, commodity, t]

        def _m_xwmg(exporter, commodity, importer, t="base"):
            if if_sub:
                return model.tmarg[exporter, commodity, importer, t] * model.xw[exporter, commodity, importer, t]
            return model.xwmg[exporter, commodity, importer, t]

        def _m_xmgm(mode, exporter, commodity, importer, t="base"):
            if if_sub:
                share = model.amgm[mode, exporter, commodity, importer]
                lambdamg = model.lambdamg[mode, exporter, commodity, importer] + 1e-12
                return share * _m_xwmg(exporter, commodity, importer, t) / lambdamg
            return model.xmgm[mode, exporter, commodity, importer, t]

        def _m_pwmg(exporter, commodity, importer, t="base"):
            if if_sub:
                return sum(
                    model.amgm[m, exporter, commodity, importer] * model.ptmg[m, t]
                    / (model.lambdamg[m, exporter, commodity, importer] + 1e-12)
                    for m in model.m
                )
            return model.pwmg[exporter, commodity, importer, t]

        def _m_pefob(exporter, commodity, importer, t="base"):
            if if_sub:
                ...
                return (1.0 + export_tax + etax) * model.pe[exporter, commodity, importer, t]
            return model.pefob[exporter, commodity, importer, t]

        def _m_pmcif(exporter, commodity, importer, t="base"):
            if if_sub:
                tmarg = model.tmarg[exporter, commodity, importer, t]
                return _m_pefob(exporter, commodity, importer, t) + _m_pwmg(exporter, commodity, importer, t) * tmarg
            return model.pmcif[exporter, commodity, importer, t]

        def _m_pm(exporter, commodity, importer, t="base"):
            if if_sub:
                ...
                return ((1.0 + imptx + mtax) * _m_pmcif(exporter, commodity, importer, t)) / chipm
            return model.pm[exporter, commodity, importer, t]
        ...
        def _m_pfy(region, factor, activity, t=None):
            ...
            return model.pfy[region, factor, activity, t]
        ...
        def prf_y_rule(model, r, a, t):
            return model.px[r, a, t] == model.pp[r, a, t]
        model.prf_y = Constraint(model.r, model.a, model.t, rule=prf_y_rule)
        ...
        def eq_nd_rule(model, r, a, t):
            ...
            return model.nd[r, a, t] == and_val * model.xp[r, a, t] * ratio**sigmap * shift**(sigmap - 1)
        model.eq_nd = Constraint(model.r, model.a, model.t, rule=eq_nd_rule)

        # GAMS: va(r,a,t) =e= ava(r,a,t)*xp(r,a,t)*(px(r,a,t)/pva(r,a,t))**sigmap(r,a)
        #                      * (axp(r,a,t)*lambdava(r,a,t))**(sigmap(r,a)-1)
        def eq_va_rule(model, r, a, t):
            ...
            return model.va[r, a, t] == ava_val * model.xp[r, a, t] * ratio**sigmap * shift**(sigmap - 1)
        model.eq_va = Constraint(model.r, model.a, model.t, rule=eq_va_rule)
        ...
        def eq_pxeq_rule(model, r, a, t):
            ...
            sigmap = self._get_sigmap(r, a)
            expo = 1.0 - sigmap
            # Add 0*xp as a structural anchor so Hopcroft-Karp bipartite matching
            # pairs eq_pxeq with xp (the MCP complement of the zero-profit condition).
            # Without this, the matcher may leave eq_pxeq unmatched when all of its
            # price-space adjacency vars (px, pnd, pva) are claimed by other equations.
            # The term has no numerical effect — it cancels — but is visible to the
            # symbolic Jacobian sparsity detection (identify_variables).
            structural_anchor = 0 * model.xp[r, a, t]
            if abs(expo) < 1e-8:
                nd_term = (model.pnd[r, a, t] / max(self._lambdand(r, a), 1e-8)) ** and_val if and_val > 0.0 else 1.0
                va_term = (model.pva[r, a, t] / max(self._lambdava(r, a), 1e-8)) ** ava_val if ava_val > 0.0 else 1.0
                return model.px[r, a, t] == nd_term * va_term + structural_anchor

            shift = self._axp_shift(r, a) ** (sigmap - 1.0)
            lambdand = max(self._lambdand(r, a), 1e-8)
            lambdava = max(self._lambdava(r, a), 1e-8)

            term_nd = and_val * (model.pnd[r, a, t] / lambdand) ** expo if and_val > 0.0 else 0.0
            term_va = ava_val * (model.pva[r, a, t] / lambdava) ** expo if ava_val > 0.0 else 0.0
            return model.px[r, a, t] ** expo == shift * (term_nd + term_va) + structural_anchor
        model.eq_pxeq = Constraint(model.r, model.a, model.t, rule=eq_pxeq_rule)

        # Price of ND bundle (GAMS pndeq)
        # pnd**(1-sigmand) = sum(i, io(r,i,a)*[pa(r,i)/lambdaio(r,i,a)]**(1-sigmand))
        def eq_pndeq_rule(model, r, a, t):
            ...
            if abs(expo) < 1e-8:
                ...
                for i in model.i:
                    ...
                    terms.append((model.pa[r, i, a, t] / lambdaio) ** io_val)
                if not terms:
                    return Constraint.Skip
                prod = 1.0
                for term in terms:
                    prod *= term
                return model.pnd[r, a, t] == prod
            ...
            for i in model.i:
                ...
                terms.append(io_val * (model.pa[r, i, a, t] / lambdaio) ** expo)

            if not terms:
                return Constraint.Skip
            return model.pnd[r, a, t] ** expo == sum(terms)
        model.eq_pndeq = Constraint(model.r, model.a, model.t, rule=eq_pndeq_rule)
        ...
        def eq_pvaeq_rule(model, r, a, t):
            ...
            if abs(expo) < 1e-8:
                ...
                return model.pva[r, a, t] * model.va[r, a, t] == sum(
            ...
            return model.pva[r, a, t] ** expo == sum(terms)
        model.eq_pvaeq = Constraint(model.r, model.a, model.t, rule=eq_pvaeq_rule)

        # Output allocation (GAMS xeq)
        def eq_x_rule(model, r, a, i, t):
            outputs = self.sets.activity_commodities.get(a, [])
            if outputs and i not in outputs:
                return Constraint.Skip
            ...
            omega = self._get_omegas(r, a)
            if omega == float("inf"):
                return model.p_rai[r, a, i, t] == model.px[r, a, t]

            return model.x[r, a, i, t] == share * (model.xp[r, a, t] / model.xscale[r, a]) * (model.p_rai[r, a, i, t] / model.px[r, a, t]) ** omega
        model.eq_x = Constraint(model.r, model.a, model.i, model.t, rule=eq_x_rule)

        def eq_po_rule(model, r, a, t):
            outputs = self.sets.activity_commodities.get(a)
            if not outputs:
                return Constraint.Skip
            ...
            if omega == float("inf"):
                ...
                return (model.xp[r, a, t] / model.xscale[r, a]) == sum(model.x[r, a, i, t] for i in active_outputs)

            exponent = 1.0 + omega
            return model.px[r, a, t] ** exponent == sum(
                value(model.gx_param[r, a, i]) * model.p_rai[r, a, i, t] ** exponent
                for i in active_outputs
            )
        model.eq_po = Constraint(model.r, model.a, model.t, rule=eq_po_rule)

        # Commodity-level output tax mapping (GAMS ppeq structure):
        # pp(r,a,i) = (1 + prdtx(r,a,i)) * p(r,a,i)
        def eq_pp_rai_rule(model, r, a, i, t):
            outputs = self.sets.activity_commodities.get(a, [])
            if outputs and i not in outputs:
                return Constraint.Skip
            if value(model.xflag[r, a, i]) <= 0.0:
            ...
            if share <= 0.0 and make_base <= 0.0:
                return Constraint.Skip
            return model.pp_rai[r, a, i, t] == (1.0 + value(model.prdtx_rai[r, a, i])) * model.p_rai[r, a, i, t]
        model.eq_pp_rai = Constraint(model.r, model.a, model.i, model.t, rule=eq_pp_rai_rule)

        # Commodity aggregation by make-route (GAMS peq)
        def eq_peq_rule(model, r, a, i, t):
            outputs = self.sets.activity_commodities.get(a, [])
            if outputs and i not in outputs:
                return Constraint.Skip
            if value(model.xflag[r, a, i]) <= 0.0:
            ...
            if sigma == float("inf"):
                return _m_pp(r, a, i, t) == model.ps[r, i, t]

            return model.x[r, a, i, t] == ax_val * model.xs[r, i, t] * (model.ps[r, i, t] / _m_pp(r, a, i, t)) ** sigma
        model.eq_peq = Constraint(model.r, model.a, model.i, model.t, rule=eq_peq_rule)
        ...
        def eq_xs_rule(model, r, i, t):
            ...
            sigma = self._get_sigmas(r, i)
            if sigma == float("inf"):
                return model.xs[r, i, t] == sum(model.x[r, a, i, t] for a in active_activities)

            exponent = 1.0 - sigma
            if abs(exponent) < 1e-8:
                return Constraint.Skip
            return model.ps[r, i, t] ** exponent == sum(
                value(model.p_ax[r, a, i]) * _m_pp(r, a, i, t) ** exponent
                for a in active_activities
            )
        model.eq_xs = Constraint(model.r, model.i, model.t, rule=eq_xs_rule)

        # Legacy Pyomo simplification. The CET block should determine ps/pd/pet
        # without an extra identity constraint.
        def eq_ps_rule(model, r, i, t):
            return model.ps[r, i, t] == model.pd[r, i, t]
        model.eq_ps = Constraint(model.r, model.i, model.t, rule=eq_ps_rule)
        ...
        def eq_xds_rule(model, r, i, t):
            omega = self.params.elasticities.omegax.get((r, i), float("inf"))
            gd_share = value(model.gd_share[r, i])
            if gd_share <= 0.0:
                return model.xds[r, i, t] == 0.0
            if omega == float("inf"):
                return model.pd[r, i, t] == model.ps[r, i, t]
            return model.xds[r, i, t] == model.gd_share[r, i] * model.xs[r, i, t] * (model.pd[r, i, t] / model.ps[r, i, t]) ** omega
        model.eq_xds = Constraint(model.r, model.i, model.t, rule=eq_xds_rule)

        def eq_xet_rule(model, r, i, t):
            omega = self.params.elasticities.omegax.get((r, i), float("inf"))
            if value(model.xet_flag[r, i]) <= 0.0:
                return Constraint.Skip
            ge_share = value(model.ge_share[r, i])
            if ge_share <= 0.0:
                return model.xet[r, i, t] == 0.0
            if omega == float("inf"):
                return model.pet[r, i, t] == model.ps[r, i, t]
            return (
                model.xet[r, i, t]
                == model.ge_share[r, i]
                * model.xs[r, i, t]
                * (model.pet[r, i, t] / model.ps[r, i, t]) ** omega
            )
        model.eq_xet = Constraint(model.r, model.i, model.t, rule=eq_xet_rule)

        def eq_xseq_rule(model, r, i, t):
            omega = self.params.elasticities.omegax.get((r, i), float("inf"))
            gd_share = value(model.gd_share[r, i])
            ge_share = value(model.ge_share[r, i])
            if omega == float("inf"):
                return model.xs[r, i, t] == model.xds[r, i, t] + model.xet[r, i, t]
            exponent = 1.0 + omega
            return (
                model.ps[r, i, t] ** exponent
                == model.gd_share[r, i] * model.pd[r, i, t] ** exponent
                + model.ge_share[r, i] * model.pet[r, i, t] ** exponent
            )
        model.eq_xseq = Constraint(model.r, model.i, model.t, rule=eq_xseq_rule)

        # Legacy Pyomo simplification. Aggregate export price should be governed
        # by peeq/peteq, not an extra pet == ps identity.
        def eq_pe_rule(model, r, i, t):
            return model.pet[r, i, t] == model.ps[r, i, t]
        model.eq_pe = Constraint(model.r, model.i, model.t, rule=eq_pe_rule)
        ...
        def eq_pe_route_rule(model, r, i, rp, t):
            if r == rp:
                return Constraint.Skip
            bilateral_exports = self.params.benchmark.vxmd.get((r, i, rp), 0.0)
            mirror_imports = self.params.benchmark.viws.get((rp, i, r), 0.0)
            if bilateral_exports <= 0.0 and mirror_imports <= 0.0:
                return Constraint.Skip
            return model.pe[r, i, rp, t] == model.pet[r, i, t]
        model.eq_pe_route = Constraint(model.r, model.i, model.rp, model.t, rule=eq_pe_route_rule)
        ...
        def eq_xet_agg_rule(model, r, i, t):
            active_partners = [rp for rp in model.rp if rp != r and self.params.benchmark.vxmd.get((r, i, rp), 0.0) > 0.0]
            if not active_partners:
                return model.xet[r, i, t] == 0.0
            return model.xet[r, i, t] == sum(model.xw[r, i, rp, t] for rp in active_partners)
        model.eq_xet_agg = Constraint(model.r, model.i, model.t, rule=eq_xet_agg_rule)
        ...
        def eq_xe_xw_rule(model, r, i, rp, t):
            if r == rp:
                return Constraint.Skip
            bilateral_exports = self.params.benchmark.vxmd.get((r, i, rp), 0.0)
            if bilateral_exports <= 0.0:
                return Constraint.Skip
            return model.xe[r, i, rp, t] == model.xw[r, i, rp, t]
        model.eq_xe_xw = Constraint(model.r, model.i, model.rp, model.t, rule=eq_xe_xw_rule)
        ...
        for (r, i, aa), flag in xa_flag_cache.items():
            ...
            for var_name in ("xaa", "xda", "xma"):
                if not hasattr(model, var_name):
                    continue
                var = getattr(model, var_name)
                # All of xaa/xda/xma are now 4-indexed (r,i,aa,t).
                for t in self._t_set:
                    key4 = (r, i, aa, t)
                    if key4 in var:
                        var[key4].setlb(0.0)
        ...
        def eq_xaa_activity_rule(model, r, i, a, t):
            ...
            if io_val <= 0.0:
                return model.xaa[r, i, a, t] == 0.0

            sigmand = self._get_sigmand(r, a)
            lambdaio = max(value(model.lambdaio[r, i, a]), 1e-8)
            # Leontief shortcut: avoid (pnd/pa)**0 ill-conditioning.
            if abs(sigmand) < 1e-12:
                return model.xaa[r, i, a, t] == io_val * model.nd[r, a, t] / lambdaio
            return model.xaa[r, i, a, t] == (
                io_val
                * model.nd[r, a, t]
                * (model.pnd[r, a, t] / model.pa[r, i, a, t]) ** sigmand
                * (lambdaio ** (sigmand - 1.0))
            )
        model.eq_xaa_activity = Constraint(model.r, model.i, model.a, model.t, rule=eq_xaa_activity_rule)

        def eq_xaa_hhd_rule(model, r, i, t):
            if not get_xa_flag(r, i, GTAP_HOUSEHOLD_AGENT):
                return Constraint.Skip
            return model.xaa[r, i, GTAP_HOUSEHOLD_AGENT, t] == model.xc[r, i, t]
        model.eq_xaa_hhd = Constraint(model.r, model.i, model.t, rule=eq_xaa_hhd_rule)

        def eq_xaa_gov_rule(model, r, i, t):
            if not get_xa_flag(r, i, GTAP_GOVERNMENT_AGENT):
                return Constraint.Skip
            return model.xaa[r, i, GTAP_GOVERNMENT_AGENT, t] == model.xg[r, i, t]
        model.eq_xaa_gov = Constraint(model.r, model.i, model.t, rule=eq_xaa_gov_rule)

        def eq_xaa_inv_rule(model, r, i, t):
            if not get_xa_flag(r, i, GTAP_INVESTMENT_AGENT):
                return Constraint.Skip
            return model.xaa[r, i, GTAP_INVESTMENT_AGENT, t] == model.xi[r, i, t]
        ...
        def eq_xaa_tmg_rule(model, r, i, t):
            if not get_xa_flag(r, i, GTAP_MARGIN_AGENT):
                return Constraint.Skip
            i_str = str(i)
            if i_str not in margin_commodities:
                return model.xaa[r, i, GTAP_MARGIN_AGENT, t] == 0.0
            ...
            if abs(sigmamg - 1.0) < 1e-8:
                sigmamg = 1.01
            return model.xaa[r, i, GTAP_MARGIN_AGENT, t] == (
                alpha * model.xtmg[i, t] * (model.ptmg[i, t] / (model.pa[r, i, GTAP_MARGIN_AGENT, t] + 1e-12)) ** sigmamg
            )
        model.eq_xaa_tmg = Constraint(model.r, model.i, model.t, rule=eq_xaa_tmg_rule)
            ...
            for i in self.sets.i:
                for aa in all_agents:
                    if not get_xa_flag(r, i, aa):
                        ...
                        continue
                    xaa_bench = max(float(value(t0_arm.xaa[r, i, aa, "base"])) if (r, i, aa, "base") in t0_arm.xaa else 0.0, 0.0)
                    xda_bench = max(float(value(t0_arm.xda[r, i, aa, "base"])), 0.0)
                    xma_bench = max(float(value(t0_arm.xma[r, i, aa, "base"])), 0.0)
                    if xaa_bench <= 0.0 or (xda_bench <= 0.0 and xma_bench <= 0.0):
                        benchmark_agent_armington_param_cache[(r, i, aa)] = (0.0, 0.0)
                        continue

                    sigma_m = _top_armington_sigma(r, i, aa)
                    pdp_bench = float(value(t0_arm.pdp[r, i, aa, "base"]))
                    pmp_bench = float(value(t0_arm.pmp[r, i, aa, "base"]))
                    pa_bench = float(value(t0_arm.pa[r, i, aa, "base"]))
                    if pa_bench <= 0.0:
                        benchmark_agent_armington_param_cache[(r, i, aa)] = (0.0, 0.0)
                        continue
        ...
        def eq_dintxeq_rule(model, r, i, aa, t):
            def _two_key(raw_map, region, commodity):
                val = raw_map.get((region, commodity), None)
                if val is None:
                    val = raw_map.get((commodity, region), 0.0)
            ...
            return model.dintx[r, i, aa, t] == target
        model.eq_dintxeq = Constraint(model.r, model.i, model.aa, model.t, rule=eq_dintxeq_rule)

        def eq_mintxeq_rule(model, r, i, aa, t):
            def _two_key(raw_map, region, commodity):
                val = raw_map.get((region, commodity), None)
                if val is None:
                    val = raw_map.get((commodity, region), 0.0)
            ...
            return model.mintx[r, i, aa, t] == target
        model.eq_mintxeq = Constraint(model.r, model.i, model.aa, model.t, rule=eq_mintxeq_rule)

        def eq_xda_rule(model, r, i, aa, t):
            ...
            if domestic_share <= 0.0:
                return model.xda[r, i, aa, t] == 0.0
            sigma_m = _top_armington_sigma(r, i, aa)
            if sigma_m == float("inf"):
                return model.pdp[r, i, aa, t] == model.paa[r, i, aa, t]
            return model.xda[r, i, aa, t] == domestic_share * model.xaa[r, i, aa, t] * (model.paa[r, i, aa, t] / model.pdp[r, i, aa, t]) ** sigma_m
        model.eq_xda = Constraint(model.r, model.i, model.aa, model.t, rule=eq_xda_rule)

        def eq_xma_rule(model, r, i, aa, t):
            if not get_xa_flag(r, i, aa):
                return Constraint.Skip
            _, import_share = get_benchmark_agent_armington_shares(r, i, aa)
            if import_share <= 0.0:
                return model.xma[r, i, aa, t] == 0.0
            sigma_m = _top_armington_sigma(r, i, aa)
            if sigma_m == float("inf"):
                return model.pmp[r, i, aa, t] == model.paa[r, i, aa, t]
            return model.xma[r, i, aa, t] == import_share * model.xaa[r, i, aa, t] * (model.paa[r, i, aa, t] / model.pmp[r, i, aa, t]) ** sigma_m
        model.eq_xma = Constraint(model.r, model.i, model.aa, model.t, rule=eq_xma_rule)
        ...
        def eq_paa_rule(model, r, i, aa, t):
            ...
            if abs(expo) < 1e-8:  # Cobb-Douglas case
                ...
                return model.pa[r, i, aa, t] == model.pdp[r, i, aa, t] ** alphad * model.pmp[r, i, aa, t] ** alpham
            # CES case
            return model.pa[r, i, aa, t] ** expo == alphad * model.pdp[r, i, aa, t] ** expo + alpham * model.pmp[r, i, aa, t] ** expo
        model.eq_paa = Constraint(model.r, model.i, model.aa, model.t, rule=eq_paa_rule)
        ...
        def eq_xwmg_rule(model, r, i, rp, t):
            if value(model.tmarg[r, i, rp, t]) <= 0.0:
                return Constraint.Skip
            return model.xwmg[r, i, rp, t] == model.tmarg[r, i, rp, t] * model.xw[r, i, rp, t]
        model.eq_xwmg = Constraint(model.r, model.i, model.rp, model.t, rule=eq_xwmg_rule)

        def eq_xmgm_rule(model, m, r, i, rp, t):
            share = value(model.amgm[m, r, i, rp])
            if share <= 0.0:
                return Constraint.Skip
            return model.xmgm[m, r, i, rp, t] == share * model.xwmg[r, i, rp, t] / (model.lambdamg[m, r, i, rp] + 1e-12)
        model.eq_xmgm = Constraint(model.m, model.r, model.i, model.rp, model.t, rule=eq_xmgm_rule)

        def eq_pwmg_rule(model, r, i, rp, t):
            if value(model.tmarg[r, i, rp, t]) <= 0.0:
                return Constraint.Skip
            total = sum(
                model.amgm[m, r, i, rp] * model.ptmg[m, t] / (model.lambdamg[m, r, i, rp] + 1e-12)
                for m in model.m
            )
            return model.pwmg[r, i, rp, t] == total
        model.eq_pwmg = Constraint(model.r, model.i, model.rp, model.t, rule=eq_pwmg_rule)

        def eq_xtmg_rule(model, m, t):
            return model.xtmg[m, t] == sum(_m_xmgm(m, r, i, rp, t) for r in model.r for i in model.i for rp in model.rp)
        model.eq_xtmg = Constraint(model.m, model.t, rule=eq_xtmg_rule)
        ...
        def eq_ptmg_rule(model, m, t):
            i_str = str(m)
            has_supply = any(alphaa_tmg.get((str(r), i_str), 0.0) > 0.0 for r in self.sets.r)
            if not has_supply:
                return model.ptmg[m, t] == model.pnum[t]
            sigmamg = float(self.params.elasticities.sigmam.get(i_str, 1.0))
            if abs(sigmamg - 1.0) < 1e-8:
                sigmamg = 1.01
            expo = 1.0 - sigmamg
            terms = sum(
                alphaa_tmg.get((str(r), i_str), 0.0) * model.pa[r, m, GTAP_MARGIN_AGENT, t] ** expo
                for r in model.r
                if alphaa_tmg.get((str(r), i_str), 0.0) > 0.0
            )
            return model.ptmg[m, t] ** expo == terms
        model.eq_ptmg = Constraint(model.m, model.t, rule=eq_ptmg_rule)
        ...
        def eq_xweq_rule(model, rp, i, r, t):
            amw = _get_import_source_share(model, r, i, rp)
            if amw <= 0.0:
                return Constraint.Skip
            esubm = self.params.elasticities.esubm.get((r, i), 5.0)
            lambdam = _lambdam_value(rp, i, r)
            return model.xw[rp, i, r, t] == (
                amw
                * model.xmt[r, i, t]
                * (model.pmt[r, i, t] / _m_pm(rp, i, r, t)) ** esubm
                * (lambdam ** (esubm - 1.0))
            )
        model.eq_xweq = Constraint(model.rp, model.i, model.r, model.t, rule=eq_xweq_rule)
        
        # Aggregate import price CES (GAMS pmteq)
        # pmt(r,i)**(1-esubm) = sum(rp, amw(rp,i,r)*pm(rp,i,r)**(1-esubm))
        def eq_pmteq_rule(model, r, i, t):
            esubm = self.params.elasticities.esubm.get((r, i), 5.0)
            expo = 1.0 - esubm
            if abs(expo) < 1e-8:
            ...
            if not any(share > 0.0 for share in active_shares):
                return model.pmt[r, i, t] == 1.0  # Default price if no imports
            terms = []
            for rp in model.rp:
                amw = _get_import_source_share(model, r, i, rp)
                if amw <= 0.0:
                    continue
                lambdam = _lambdam_value(rp, i, r)
                terms.append(amw * (_m_pm(rp, i, r, t) / lambdam) ** expo)
            if not terms:
                return model.pmt[r, i, t] == 1.0
            return model.pmt[r, i, t] ** expo == sum(terms)
        model.eq_pmteq = Constraint(model.r, model.i, model.t, rule=eq_pmteq_rule)
        
        # Bilateral import price tariff-inclusive (GAMS pmeq)
        # pm(rp,i,r) = (1 + imptx(rp,i,r) + mtax(r,i))*pmcif(rp,i,r)
        def eq_pmeq_rule(model, rp, i, r, t):
            if value(model.xw_flag[rp, i, r]) <= 0.0:
                return Constraint.Skip
            imptx = _imptx_rate_importer(r, i, rp)
            mtax = _mtax_value(r, i, rp)
            chipm = _chipm_value(rp, i, r)
            return model.pm[rp, i, r, t] == ((1.0 + imptx + mtax) * model.pmcif[rp, i, r, t]) / chipm
        model.eq_pmeq = Constraint(model.rp, model.i, model.r, model.t, rule=eq_pmeq_rule)

        # CIF import price (GAMS pmcifeq)
        # pmcif(rp,i,r) = pefob(rp,i,r) + pwmg(rp,i,r)*tmarg(rp,i,r)
        def eq_pmcifeq_rule(model, rp, i, r, t):
            if value(model.xw_flag[rp, i, r]) <= 0.0:
                return Constraint.Skip
            tmarg = value(model.tmarg[rp, i, r, t])
            return model.pmcif[rp, i, r, t] == model.pefob[rp, i, r, t] + model.pwmg[rp, i, r, t] * tmarg
        model.eq_pmcifeq = Constraint(model.rp, model.i, model.r, model.t, rule=eq_pmcifeq_rule)

        # FOB export price (GAMS pefobeq)
        # pefob(r,i,rp) = (1 + exptx(r,i,rp) + etax(r,i))*pe(r,i,rp)
        def eq_pefobeq_rule(model, r, i, rp, t):
            if value(model.xw_flag[r, i, rp]) <= 0.0:
                return Constraint.Skip
            export_tax = float(self.params.taxes.rtxs.get((r, i, rp), 0.0))
            etax = _etax_value(r, i, rp)
            return model.pefob[r, i, rp, t] == (1.0 + export_tax + etax) * model.pe[r, i, rp, t]
        model.eq_pefobeq = Constraint(model.r, model.i, model.rp, model.t, rule=eq_pefobeq_rule)

        # Bilateral export supply CET (GAMS peeq)
        # xw(r,i,rp) = gw(r,i,rp)*xet(r,i)*(pe(r,i,rp)/pet(r,i))**omegaw(r,i)
        def eq_peeq_rule(model, r, i, rp, t):
            if value(model.xw_flag[r, i, rp]) <= 0.0:
                return Constraint.Skip
            omegaw = self.params.elasticities.omegaw.get((r, i), float("inf"))
            if omegaw == float("inf"):
            ...
            return (
                model.xw[r, i, rp, t]
                == model.gw_share[r, i, rp] * model.xet[r, i, t] * (model.pe[r, i, rp, t] / model.pet[r, i, t]) ** omegaw
            )
        model.eq_peeq = Constraint(model.r, model.i, model.rp, model.t, rule=eq_peeq_rule)
        
        # Aggregate export price CET (GAMS peteq)
        # pet(r,i)**(1+omegaw) = sum(rp, gw(r,i,rp)*pe(r,i,rp)**(1+omegaw))
        def eq_peteq_rule(model, r, i, t):
            if value(model.xet_flag[r, i]) <= 0.0:
                return Constraint.Skip
            ...
            for rp in active_routes:
                ...
                terms.append(gw * model.pe[r, i, rp, t] ** exponent)
            if not terms:
                return Constraint.Skip
            return model.pet[r, i, t] ** exponent == sum(terms)
        model.eq_peteq = Constraint(model.r, model.i, model.t, rule=eq_peteq_rule)
        ...
        def eq_pdeq_rule(model, r, i, t):
            return model.xds[r, i, t] == sum(
                model.xda[r, i, aa, t] / model.xscale[r, aa]
                for aa in model.aa
                if get_benchmark_agent_armington_shares(r, i, aa)[0] > 0.0
            )
        model.eq_pdeq = Constraint(model.r, model.i, model.t, rule=eq_pdeq_rule)
        ...
        def eq_pfyeq_rule(model, r, f, a, t):
            if value(model.xfflag[r, f, a]) <= 0.0:
                return Constraint.Skip
            kappa = float(self.params.taxes.kappaf_activity.get((r, f, a), 0.0))
            if kappa == 0.0:
                kappa = float(self.params.taxes.kappaf.get((r, f), 0.0))
            return model.pfy[r, f, a, t] == model.pf[r, f, a, t] * (1.0 - kappa)
        model.eq_pfyeq = Constraint(model.r, model.f, model.a, model.t, rule=eq_pfyeq_rule)
        ...
        def eq_kstock_rule(model, r, t):
            ...
            return krat * model.kstock[r, t] == sum(model.xft[r, f, t] for f in capital_factors)
        model.eq_kstock = Constraint(model.r, model.t, rule=eq_kstock_rule)
        ...
        def eq_xc_rule(model, r, i, t):
            ...
            if share <= 0.0:
                return model.xc[r, i, t] == 0.0
            return model.pa[r, i, "hhd", t] * model.xc[r, i, t] == model.xcshr[r, i, t] * model.yc[r, t]
        ...
        def eq_xg_rule(model, r, i, t):
            ...
            if share <= 0.0:
                return model.xg[r, i, t] == 0.0
            return model.pa[r, i, "gov", t] * model.xg[r, i, t] == share * model.yg[r, t]
        ...
        def eq_xi_rule(model, r, i, t):
            alphaa = value(model.i_share[r, i])
            if alphaa <= 0.0:
                return model.xi[r, i, t] == 0.0
            sigmai_raw = float(self.params.elasticities.esubi.get(r, 0.0))
            if abs(sigmai_raw - 1.0) < 1e-8:
                sigmai_raw = 1.01  # match GAMS cal.gms: sigmai=1 → 1.01
            return model.xi[r, i, t] == alphaa * model.xiagg[r, t] * (model.pi[r, t] / (model.pa[r, i, "inv", t] + 1e-12)) ** sigmai_raw
        model.eq_xi = Constraint(model.r, model.i, model.t, rule=eq_xi_rule)

        def eq_xiagg_rule(model, r, t):
            return model.pi[r, t] * model.xiagg[r, t] == model.yi[r, t]
        model.eq_xiagg = Constraint(model.r, model.t, rule=eq_xiagg_rule)
        ...
        # Anchored to base period (CDE calibration is static).
        def eq_zcons_rule(model, r, i, t):
            share = value(model.c_share[r, i])
            alpha = value(model.alphaa_hhd[r, i])
            if share <= 0.0 or alpha <= 0.0:
                return model.zcons[r, i, t] == 0.0
            return model.zcons[r, i, t] == (
                model.alphaa_hhd[r, i] * model.bh[r, i]
                * (model.pa[r, i, "hhd", t] ** model.bh[r, i])
                * (model.uh[r, t] ** (model.eh[r, i] * model.bh[r, i]))
                * ((model.yc[r, t] / model.pop[r]) ** (-model.bh[r, i]))
            )
        model.eq_zcons = Constraint(model.r, model.i, model.t, rule=eq_zcons_rule)

        # Household budget shares — CDE (GAMS xcshreq, model.gms:769)
        # xcshr(r,i) = zcons(r,i) / sum_j zcons(r,j)
        def eq_xcshr_rule(model, r, i, t):
            share = value(model.c_share[r, i])
            if share <= 0.0:
                return model.xcshr[r, i, t] == 0.0
            return model.xcshr[r, i, t] * sum(model.zcons[r, j, t] for j in model.i if value(model.c_share[r, j]) > 0.0) == model.zcons[r, i, t]
        model.eq_xcshr = Constraint(model.r, model.i, model.t, rule=eq_xcshr_rule)

        # CDE elasticity of expenditure wrt utility (GAMS phiPeq, model.gms:780)
        # phip = sum_i xcshr(r,i) * eh(r,i)
        # Anchored to base period.
        def eq_phip_rule(model, r, t):
            return model.phip[r, t] == sum(model.xcshr[r, i, t] * model.eh[r, i] for i in model.i if value(model.c_share[r, i]) > 0.0)
        model.eq_phip = Constraint(model.r, model.t, rule=eq_phip_rule)

        # GAMS phieq (model.gms:737): phi*(betaP/phiP + betaG + betaS) = 1
        # Anchored to base period.
        def eq_phi_rule(model, r, t):
            return model.phi[r, t] * (model.betap[r] / (model.phip[r, t] + 1e-12) + model.betag[r] + model.betas[r]) == 1.0
        model.eq_phi = Constraint(model.r, model.t, rule=eq_phi_rule)
        ...
        def eq_pi_rule(model, r, t):
            sigmai_raw = float(self.params.elasticities.esubi.get(r, 0.0))
            if abs(sigmai_raw - 1.0) < 1e-8:
                sigmai_raw = 1.01  # match GAMS cal.gms: sigmai=1 → 1.01
            expo = 1.0 - sigmai_raw
            terms = [
                value(model.i_share[r, i]) * model.pa[r, i, "inv", t] ** expo
                for i in model.i
                if value(model.i_share[r, i]) > 0.0
            ]
            if not terms:
                return model.pi[r, t] == 1.0
            return (model.axi[r] * model.pi[r, t]) ** expo == sum(terms)
        model.eq_pi = Constraint(model.r, model.t, rule=eq_pi_rule)
        ...
        def eq_uh_rule(model, r, t):
            terms = [
                model.zcons[r, i, t] / model.bh[r, i]
                for i in model.i
                if value(model.c_share[r, i]) > 0.0
            ]
            if not terms:
                return model.uh[r, t] == 1.0
            return sum(terms) == 1.0
        model.eq_uh = Constraint(model.r, model.t, rule=eq_uh_rule)
        ...
        def eq_ug_rule(model, r, t):
            ...
            if abs(sigmag - 1.0) < 1e-8:
                ...
                for i, s in active_shares:
                    pg_index = pg_index * model.pa[r, i, "gov", t] ** s
                return model.ug[r, t] * model.pop[r] * pg_index == model.aug[r] * model.yg[r, t]
            expo = 1.0 - sigmag
            pg_terms = sum(s * model.pa[r, i, "gov", t] ** expo for i, s in active_shares)
            pg_index = pg_terms ** (1.0 / expo)
            return model.ug[r, t] * model.pop[r] * pg_index == model.aug[r] * model.yg[r, t]
        model.eq_ug = Constraint(model.r, model.t, rule=eq_ug_rule)
        ...
        def eq_us_rule(model, r, t):
            return model.us[r, t] == model.aus[r] * model.rsav[r, t] / (model.psave[r, t] * model.pop[r] + 1e-12)
        model.eq_us = Constraint(model.r, model.t, rule=eq_us_rule)
        ...
        def eq_u_rule(model, r, t):
            expr = model.au[r]
            if float(value(model.betap[r])) != 0.0:
                expr = expr * (model.uh[r, t] ** model.betap[r])
            if float(value(model.betag[r])) != 0.0:
                expr = expr * (model.ug[r, t] ** model.betag[r])
            if float(value(model.betas[r])) != 0.0:
                expr = expr * (model.us[r, t] ** model.betas[r])
            return model.u[r, t] == expr
        model.eq_u = Constraint(model.r, model.t, rule=eq_u_rule)

        # Savings price adjustment (GAMS chiSaveeq, compStat-style static form)
        def eq_chisave_rule(model, t):
            numer = sum(model.invwgt[r] * model.pi[r, t] for r in model.r)
            ...
            denom = sum(model.savwgt[r] * model.pi[r, t] for r in model.r)
            return (model.chiSave[t] ** 2) * denom == numer
        model.eq_chisave = Constraint(model.t, rule=eq_chisave_rule)

        def eq_pigbl_rule(model, t):
            net_inv_value = sum(
                model.pi[r, t] * (model.xiagg[r, t] - model.depr[r] * model.kstock[r, t])
                for r in model.r
            )
            return model.pigbl[t] * model.xigbl[t] == net_inv_value
        model.eq_pigbl = Constraint(model.t, rule=eq_pigbl_rule)

        # Global net investment (GAMS xigbleq)
        def eq_xigbl_rule(model, t):
            return model.xigbl[t] == sum(
                model.xiagg[r, t] - model.depr[r] * model.kstock[r, t]
                for r in model.r
            )
        model.eq_xigbl = Constraint(model.t, rule=eq_xigbl_rule)

        # Foreign savings (GAMS savfeq)
        def eq_savf_rule(model, r, t):
            savf_flag = getattr(self.closure, "savf_flag", "capFix") if self.closure else "capFix"
            ...
            if savf_flag == "capSFix":
                if is_residual:
                    return Constraint.Skip
                return model.savf[r, t] == model.chif[r, t] * model.regy[r, t]
            return model.savf[r, t] == model.chif[r, t] * model.regy[r, t]
        model.eq_savf = Constraint(model.r, model.t, rule=eq_savf_rule)

        # Share of nominal foreign savings in regional income (GAMS chifeq)
        def eq_chif_rule(model, r, t):
            if self.closure and getattr(self.closure, "savf_flag", "capFix") == "capSFix":
                return model.chif[r, t] == model.chif0[r]
            return model.savf[r, t] == model.chif[r, t] * model.regy[r, t]
        model.eq_chif = Constraint(model.r, model.t, rule=eq_chif_rule)

        # Capital account balance (GAMS capAccteq)
        def eq_capacct_rule(model, t):
            return sum(model.savf[r, t] for r in model.r) == 0.0
        model.eq_capAcct = Constraint(model.t, rule=eq_capacct_rule)

        # Rate of return block (GAMS arent/rorc/rore/kapEnd) — anchored to base period
        def eq_kapend_rule(model, r, t):
            if t != "base":
                return Constraint.Skip
            return model.kapEnd[r, t] == (1.0 - model.depr[r]) * model.kstock[r, t] + model.xiagg[r, t]
        model.eq_kapEnd = Constraint(model.r, model.t, rule=eq_kapend_rule)

        def eq_arent_rule(model, r, t):
            if t != "base":
                return Constraint.Skip
            capital_factors = [f for f in model.f if str(f).lower() in ("capital", "cap", "k", "kap")]
            if not capital_factors:
                return model.arent[r, t] == 0.0
            cap_return = 0.0
            for f in capital_factors:
                for a in model.a:
                    kappa = float(self.params.taxes.kappaf_activity.get((r, f, a), 0.0))
                    cap_return += (1.0 - kappa) * model.pf[r, f, a, t] * model.xf[r, f, a, t] / model.xscale[r, a]
            return model.arent[r, t] == cap_return / (model.kstock[r, t] + 1e-12)
        model.eq_arent = Constraint(model.r, model.t, rule=eq_arent_rule)

        def eq_rorc_rule(model, r, t):
            if t != "base":
                return Constraint.Skip
            return model.rorc[r, t] == model.arent[r, t] / (model.pi[r, t] + 1e-12) - model.fdepr[r]
        model.eq_rorc = Constraint(model.r, model.t, rule=eq_rorc_rule)

        def eq_rore_rule(model, r, t):
            if t != "base":
                return Constraint.Skip
            return model.rore[r, t] == model.rorc[r, t] * (model.kstock[r, t] / (model.kapEnd[r, t] + 1e-12)) ** model.rorflex[r]
        model.eq_rore = Constraint(model.r, model.t, rule=eq_rore_rule)

        # Global rate of return (GAMS rorgeq, weighted by net investment)
        def eq_rorg_rule(model, t):
            numer = 0.0
            denom = 0.0
            for r in model.r:
        ...
        def eq_facty_rule(model, r, t):
            return model.facty[r, t] == (
                sum(model.pf[r, f, a, t] * model.xf[r, f, a, t] / model.xscale[r, a] for f in model.f for a in model.a)
                - model.fdepr[r] * model.pi[r, t] * model.kstock[r, t]
            )
        model.eq_facty = Constraint(model.r, model.t, rule=eq_facty_rule)

        # Tax revenues by stream (GAMS ytaxeq)
        def eq_ytax_rule(model, r, gy, t):
            if gy == "pt":
                total = 0.0
                for a in model.a:
                    for i in self.sets.activity_commodities.get(a, list(model.i)):
                        if value(model.xflag[r, a, i]) <= 0.0:
                            continue
                        total += model.prdtx_rai[r, a, i] * model.p_rai[r, a, i, t] * model.x[r, a, i, t]
                return model.ytax[r, gy, t] == total
            ...
            if gy in ("fc", "pc", "gc", "ic"):
                ...
                for aa in agents:
                    for i in model.i:
                        ...
                        total += dintx * model.pd[r, i, t] * model.xda[r, i, aa, t] / scale
                        total += mintx * model.pmt[r, i, t] * model.xma[r, i, aa, t] / scale
                return model.ytax[r, gy, t] == total
            ...
            if gy == "et":
                ...
                for (rr, i, rp), rtxs in self.params.taxes.rtxs.items():
                    if rr != r:
                        continue
                    etax = _etax_value(r, i, rp)
                    total += (float(rtxs) + etax) * model.pe[r, i, rp, t] * model.xw[r, i, rp, t]
                return model.ytax[r, gy, t] == total
            ...
            if gy == "mt":
                ...
                for (exporter, i, importer), imptx in self.params.taxes.imptx.items():
                    ...
                    total += (float(imptx) + mtax) * _m_pmcif(exporter, i, r, t) * model.xw[exporter, i, r, t]
                return model.ytax[r, gy, t] == total
            ...
            if gy == "dt":
                ...
                for f in model.f:
                    for a in model.a:
                        ...
                        if kappa == 0.0:
                            continue
                        total += kappa * model.pf[r, f, a, t] * model.xf[r, f, a, t] / model.xscale[r, a]
                return model.ytax[r, gy, t] == total
        ...
        # eq_regy at t in model.ts (i.e. NOT at the base period). Constraint.Skip
        # at base handles the activation guard — no explicit .deactivate() needed.
        ...
        def eq_ytaxshreq_rule(model, r, gy, t):
            return model.ytaxshr[r, gy, t] == model.ytax[r, gy, t] / (model.regy[r, t] + 1e-12)
        model.eq_ytaxshreq = Constraint(model.r, model.gy, model.t, rule=eq_ytaxshreq_rule)
        ...
        def eq_yi_rule(model, r, t):
            if str(r) in residual_regions:
                return Constraint.Skip
            return model.yi[r, t] == model.pi[r, t] * model.depr[r] * model.kstock[r, t] + model.rsav[r, t] + model.savf[r, t]
        model.eq_yi = Constraint(model.r, model.t, rule=eq_yi_rule)
        ...
        base_pa = {
            (r, i, agent): float(value(t0.pa[r, i, agent, "base"]))
            for r in model.r
            for i in model.i
            for agent in final_demand_agents
        }
        base_xaa = {
            (r, i, agent): float(value(t0.xaa[r, i, agent, "base"]))
            for r in model.r
            for i in model.i
            for agent in final_demand_agents
            if (r, i, agent, "base") in t0.xaa
        }
        base_pefob = {
            (r, i, rp): float(value(t0.pefob[r, i, rp, "base"]))
            for r in model.r
            for i in model.i
            for rp in model.rp
        }
        base_pmcif = {
            (rp, i, r): float(value(t0.pmcif[rp, i, r, "base"]))
            for rp in model.rp
            for i in model.i
            for r in model.r
        }
        base_xw = {
            (r, i, rp): float(value(t0.xw[r, i, rp, "base"]))
            for r in model.r
            for i in model.i
            for rp in model.rp
        }
        ...
        def _mqabs(model, region, *, price_base: bool, quantity_base: bool):
            ...
            for i in model.i:
                for agent in final_demand_agents:
                    pa = base_pa[(region, i, agent)] if price_base else model.pa[region, i, agent, "base"]
                    xa = base_xaa.get((region, i, agent), 0.0) if quantity_base else (model.xaa[region, i, agent, "base"] if (region, i, agent, "base") in model.xaa else 0.0)
                    total += pa * xa
            return total
        ...
        def _mqtrade(model, region, *, price_base: bool, quantity_base: bool):
            ...
            for i in model.i:
                for rp in model.rp:
                    ...
                    xexp = base_xw[(region, i, rp)] if quantity_base else model.xw[region, i, rp, "base"]
                    ximp = base_xw[(rp, i, region)] if quantity_base else model.xw[rp, i, region, "base"]
        ...
        # EV anchored to base-period utility — intentional.
        def eq_ev_rule(model, r, t):
            if t != "base":
                return Constraint.Skip
            terms = []
            for i in model.i:
                ...
                terms.append(
                    alpha
                    * (model.uh[r, "base"] ** (bh * eh))
                    * ((model.pop[r] / model.ev[r, t]) ** bh)
                )
            if not terms:
                return Constraint.Skip
            return sum(terms) == 1.0
        model.eq_ev = Constraint(model.r, model.t, rule=eq_ev_rule)
        # GAMS keeps eveq active so welfare ev tracks shock state. Activate to match.

        def eq_cv_rule(model, r, t):
            if t != "base":
                return Constraint.Skip
            terms = []
            for i in model.i:
                ...
                terms.append(
                    alpha
                    * ((model.pa[r, i, GTAP_HOUSEHOLD_AGENT, "base"] * model.pop[r] / model.cv[r, t]) ** bh)
                )
            if not terms:
                return Constraint.Skip
            return sum(terms) == 1.0
        model.eq_cv = Constraint(model.r, model.t, rule=eq_cv_rule)
            ...
            for s in self._rmuv:
                for j in self._imuv:
                    for d in self.sets.r:
                        ...
                        try:
                            xw0_data[(s, j, d)] = float(value(model.xw[s, j, d, "base"]))
                        except Exception:
                            xw0_data[(s, j, d)] = 0.0
        ...
        def eq_walras_rule(model, t):
            ...
            return model.walras[t] == sum(
                model.yi[r, t] - (model.pi[r, t] * model.depr[r] * model.kstock[r, t] + model.rsav[r, t] + model.savf[r, t])
        ...
        if if_sub:
            ...
            # Fix substituted vars for ALL periods. When if_sub=True the defining
            # equations (eq_pefobeq, eq_pmcifeq, eq_pmeq, eq_pfaeq, eq_pfyeq,
            # eq_xwmg, eq_xmgm, eq_pwmg, eq_pp_rai) are deactivated AND the
            # corresponding Vars are fully substituted out of all other equation
            # bodies. So at non-base periods these Vars are orphaned free vars
            # with no equation → must be fixed to keep PATH's system square.
            for t in self._t_set:
                for r in model.r:
                    for a in model.a:
                        for i in model.i:
                            if (r, a, i, t) not in model.pp_rai:
                                continue
                            _fix_component(model.pp_rai[r, a, i, t], value(_m_pp(r, a, i, t)))

                for r in model.r:
                    for i in model.i:
                        for rp in model.rp:
                            if (r, i, rp, t) in model.xwmg:
                                _fix_component(model.xwmg[r, i, rp, t], value(_m_xwmg(r, i, rp, t)))
                            if (r, i, rp, t) in model.pwmg:
                                _fix_component(model.pwmg[r, i, rp, t], value(_m_pwmg(r, i, rp, t)))
                            if (r, i, rp, t) in model.pefob:
                                _fix_component(model.pefob[r, i, rp, t], value(_m_pefob(r, i, rp, t)))
                            if (r, i, rp, t) in model.pmcif:
                                _fix_component(model.pmcif[r, i, rp, t], value(_m_pmcif(r, i, rp, t)))
                            if (r, i, rp, t) in model.pm:
                                _fix_component(model.pm[r, i, rp, t], value(_m_pm(r, i, rp, t)))
                            for m in model.m:
                                if (m, r, i, rp, t) in model.xmgm:
                                    _fix_component(model.xmgm[m, r, i, rp, t], value(_m_xmgm(m, r, i, rp, t)))

            for r in model.r:
                for f in model.f:
                    for a in model.a:
                        for t in self._t_set:
                            # Fix pfa for ALL periods (eq_pfaeq is deactivated; without
                            # this pfa[r,f,a,t] is free with no defining equation at
                            # non-base periods, creating unmatched free vars for PATH).
                            if (r, f, a, t) in model.pfa:
                                _fix_component(model.pfa[r, f, a, t], value(_m_pfa(r, f, a, t)))
