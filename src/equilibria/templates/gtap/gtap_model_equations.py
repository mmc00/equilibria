...
from equilibria.templates.gtap.gtap_benchmark_scaling import (
    ScalingContext as _ScalingContext,
)
from equilibria.templates.gtap.gtap_benchmark_scaling import (
    _refresh_cet_share_state,
    _refresh_macro_initial_state,
)
from equilibria.templates.gtap.gtap_benchmark_scaling import (
    align_xi_xaa_post_scaling as _align_xi_xaa_post_scaling,
)
from equilibria.templates.gtap.gtap_benchmark_scaling import (
    apply_production_scaling as _apply_production_scaling,
)
...
class GTAPModelEquations:
    ...
    # --- escalado de benchmark -------------------------------------------
    # Las cuatro funciones viven en ``gtap_benchmark_scaling``: el composer de
    # bloques las necesita y antes construia un GTAPModelEquations entero solo
    # para tomarlas prestadas. Estos delegadores mantienen intacta la API del
    # monolito, que sigue siendo el oraculo de paridad contra GAMS.

    def _scaling_context(self) -> _ScalingContext:
    ...
    def _align_xi_xaa_post_scaling(self, model: ConcreteModel) -> None:
        _align_xi_xaa_post_scaling(model, self._scaling_context())

    def apply_production_scaling(self, model: ConcreteModel) -> None:
        _apply_production_scaling(model, self._scaling_context())

    def _refresh_macro_initial_state(self, model: ConcreteModel) -> None:
        _refresh_macro_initial_state(model, self._scaling_context())

    def _refresh_cet_share_state(self, model: ConcreteModel) -> None:
        _refresh_cet_share_state(model, self._scaling_context())
