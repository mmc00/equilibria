...
def deactivate_unmatched_xseq(model, params, *, label: str = "") -> int:
    ...
    if label:
        unmatched_eqs = [
            cons_snap[u].name for u in range(n) if pl[u] == -1 and cons_snap[u].active
        ]
        unmatched_vars = [
            vars_snap[v].name
            for v in range(nv)
        ...
        if unmatched_eqs:
            print(
                f"[{label}] unmatched active eqs ({len(unmatched_eqs)}): {unmatched_eqs[:8]}"
            )
        if unmatched_vars:
            print(
                f"[{label}] unmatched free vars ({len(unmatched_vars)}): {unmatched_vars[:8]}"
            )
    return deact
...
def deactivate_zero_unique_var_eqs(model, *, label: str = "") -> int:
    ...
    def _force_pair(eq_comp_name: str, var_comp_name: str) -> None:
        ...
        for idx in comp_eq:
            ...
            eq_name = con.name
            idx_str = (
                ",".join(str(x) for x in idx) if isinstance(idx, tuple) else str(idx)
            )
    ...
    for u in range(n):
        ...
        if has_unique:
            if label:
                print(
                    f"[{label}] WARN: skipping deactivation of {c.name} — has unique var"
                )
            continue
        c.deactivate()
        deact += 1
...
def apply_squareness_patches(
    model, params, *, label: str = "", protect_xseq: bool = False
) -> dict[str, int]:
    ...
    return {
        ...
        "xseq_deact": (
            0 if protect_xseq else deactivate_unmatched_xseq(model, params, label=label)
        ),
    ...
    if forced_pairs:
        for _fp in forced_pairs:
            ...
            if r is None or c is None:
                if label:
                    print(
                        f"[{label}] matching WARN: forced pair {eq_name}<->{var_name} not found"
                    )
                continue
            if c not in adjacency[r] and not hard:
                if label:
                    print(
                        f"[{label}] matching WARN: forced pair {eq_name}<->{var_name} not in adjacency"
                    )
                continue
            pair_left[r] = c
            pair_right[c] = r
