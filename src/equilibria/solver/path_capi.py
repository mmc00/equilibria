"""PATH C-API nonlinear MCP solve for the GTAP model.

Moved VERBATIM from ``scripts/gtap/run_gtap.py`` (L1063-1589, L2322-5557) so
that the solver travels inside the installed package. Before this move
``gtap_multiperiod_driver`` loaded it by path from the repo's ``scripts/``
directory, which is not part of the wheel: any pip-installed run raised
``FileNotFoundError`` on the first solve.

The function bodies are unchanged — only the module-level names they read
(constants, ``logger``, the ``GTAP_*_AGENT`` labels) are re-imported here.
``run_gtap.py`` re-imports these names, so its own callers keep working.
"""
...
# Opt-in GAMS comparison snapshot. Excluded from the wheel (it is a parity
# input, not library data), so this path may legitimately not exist.
COMP_CSV_REFERENCE = (
...
def _default_jacobian_eval_mode() -> str:
    ...
    In ``run_gtap.py`` this was a module-level constant, which worked only
    because the module was loaded by path *inside* the solve call — i.e. after
    any ``monkeypatch.setenv``. Now that the solver is imported normally, an
    import-time constant would freeze the default before callers can set the
    variable (test_multiperiod_driver.py needs "reverse_numeric" and would
    silently get "asl": ASL aborts on this ifSUB model, success=False, values
    never written back, and the Fisher row shows the residual of an UNSOLVED
    model).
    """
...
def _default_ipopt_required_infeasibility_reduction() -> float:
    ...
    value before a caller can set the variable)."""
    return float(os.environ.get("EQUILIBRIA_GTAP_IPOPT_REQ_INFEAS", "0.99"))
...
def _run_path_capi_nonlinear_full(
    ...
    *,
    ...
    path_license_string: str | None = None,
    ...
    closure_config: GTAPClosureConfig | None = None,
    x0_floor: float | None = 1e-8,
    jacobian_eval_mode: str | None = None,
    residual_trace_enabled: bool = False,
    residual_trace_max_calls: int = 120,
    residual_trace_top_n: int = 12,
    residual_trace_focus_patterns: list[str] | None = None,
    residual_trace_file: Path | None = None,
    ...
    try:
        from path_capi_python import (  # type: ignore
            PATHLoader,
            PyomoMCPAdapter,
            solve_nonlinear_mcp,
        )
    ...
    if _is_altertax_closure and hasattr(model, "eq_xft"):
        _mf_set = {str(f) for f in model.mf} if hasattr(model, "mf") else set()
        _xft_deact = 0
        for _r in model.r:
            for _f in model.f:
    ...
    from equilibria.solver._closure_patches import apply_squareness_patches
    ...
    if len(constraints) == len(free_variables):
        ...
        if os.environ.get("EQUILIBRIA_GTAP_DIRECT_DEGEN_FIX") == "1":
            ...
            _cons_set = {id(c) for c in constraints}
            _drop_cons = set()
            _drop_vars = set()
            _n_fixed_dg = 0
            for _c in list(constraints):
                _body_vars = [
                    _v for _v in _idv_dg(_c.body, include_fixed=False) if not _v.fixed
                ]
                    ...
                    if _vb in ("xds", "xet", "xi", "xft"):
                        ...
                        if _is_trivial_zero:
                            try:
                                ...
                                pass
            if _drop_cons:
                constraints = [c for c in constraints if id(c) not in _drop_cons]
                free_variables = [v for v in free_variables if id(v) not in _drop_vars]
                print(
    ...
    if _nlp_solve_report_path:
        ...
        _con_set = {id(c) for c in constraints}
        _var_set = {id(v) for v in free_variables}
        ...
        if _scaled_nl_export_path:
            ...
            if _seed_gdx:
                ...
                # Opt-in GDX-seeding oracle: _diff_core is a parity helper that
                # lives in the repo's scripts/gtap, not in the package. Only
                # reachable when EQUILIBRIA_SEED_FROM_GDX is set, i.e. from a
                # source checkout.
                _sys_s.path.insert(
                    0,
                    str(Path(__file__).resolve().parents[3] / "scripts" / "gtap"),
                )
        ...
        if os.environ.get("EQUILIBRIA_DEBUG_NLP_VIA_GAMS"):
            ...
            if _method == "ptc":
                ...
                from scipy.sparse import eye as _eye_tr  # noqa: F401
                from scipy.sparse import identity as _id_tr
                ...
                def _paired_colperm():
                    try:
                        ...
                        _matched = 0
                        for _c_obj, _v_obj in zip(
                            constraints, free_variables, strict=False
                        ):
                        ...
                        _used = {int(x) for x in _perm if x >= 0}
                ...
                _cudss_solver_tr = (
                    None  # persists cuDSS plan across Newton steps (reuse)
                )
                ...
                for _k_tr in range(_maxit_tr):
                    ...
                    if _linsolve_tr == "gmres":
                        ...
                        if _gmin > 0.0:
                            ...
                            if (
                                os.environ.get("EQUILIBRIA_GTAP_GMIN_SYM_REUSE", "1")
                                != "0"
                            ):
                        ...
                        if _need_lu or _gmin > 0.0:
                            try:
                                ...
                                if _lu_tr is None:
                                    ...
                                    _icntl7 = int(
                                        os.environ.get(
                                            "EQUILIBRIA_GTAP_MUMPS_ICNTL7", "0"
                                        )
                                    )  # 0=AMD (safe, avoids PORD segfault)
                                    _icntl8 = int(
                                        os.environ.get(
                                            "EQUILIBRIA_GTAP_MUMPS_ICNTL8", "8"
                                        )
                                    ...
                                    if _nnull:
                                        ...
                                        if (
                                            ...
                                            try:
                                                ...
                                                _plog_m(f"nullpiv naming failed: {_ne}")
                                except Exception:
                                    pass
                                _plog_m(
                        ...
                        if _colperm_tr is None:
                            ...
                            if _pp_c is not None and _pc_matched >= _n_sr:
                                ...
                                _colperm_tr = _mbm_c(_J_tr.tocsr(), perm_type="column")
                                if _np_sr.any(_colperm_tr < 0):
                    ...
                    if _k_tr % 5 == 0:
                        _relstr = ""
                        if _fscale_tr is not None:
                            _relstr = f" ||F/scale||={float(_np_sr.linalg.norm(_F_tr / _fscale_tr, _np_sr.inf)):.3e}"
                        print(
            ...
            for _v_sr, _val_sr in zip(
                _nlp_sr.get_pyomo_variables(), _sol_sr.x, strict=False
            ):
                if not _v_sr.fixed:
    ...
    if x0_floor is not None:
        for i, (x0, lb, ub) in enumerate(zip(data.x0, data.lb, data.ub, strict=False)):
            x0_adj = x0
            if lb > -1.0e19:
    ...
    if equation_scaling:
        ...
        for _j, (_start, _len) in enumerate(
            zip(
                data.jacobian_structure.col_starts,
                data.jacobian_structure.col_lengths,
                strict=False,
            )
        ):
    ...
    for name, value in zip(data.variable_names, _solution_x, strict=False):
